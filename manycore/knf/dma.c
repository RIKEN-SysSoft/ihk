                       ...
                       unsigned long value, int intr)
{
	...
	desc->desc.memcpy.sap = (unsigned long)src;
	desc->desc.memcpy.dap = (unsigned long)dest;
	desc->desc.memcpy.length = size >> 6; /* 2^6 = 64 */

	if (ndesc > 1) {
		desc = __knf_desc_proceed_head(c);
		desc->desc.status.type = 2;
		if (!intr && priv) {
			desc->desc.status.dap = (unsigned long)priv;
			desc->desc.status.data = value;
		} else {
