...
int ihk_reserve_mem_conf(int index, int key, void *value)
{
	...
	case IHK_RESERVE_MEM_BALANCED_ENABLE:
		reserve_mem_conf.balanced_enable = *((int *)value);
		    ...
		    int num_mem_chunks)
{
	...
	int release = 0;
#ifdef WITH_KRM
	int *cgroup_nodeids = NULL;
	int nr_cgroup_nodeids;
#endif
	...
	ret = parse_cgroup_set("/sys/fs/cgroup/cpuset/pxkrmjobs.slice/cpuset.mems",
			      &cgroup_nodeids, &nr_cgroup_nodeids);
	if (ret) {
		dprintf("%s: error: parse_cgroup_set failed with %d\n",
			__func__, ret);
		goto out;
	...
	for (i = 0; i < num_mem_chunks; i++) {
		...
		for (j = 0; j < nr_cgroup_nodeids; j++) {
			if (mem_chunks[i].numa_node_number == cgroup_nodeids[j]) {
				found = 1;
				break;
			}
		}
...
#define IHKLIB_RESERVE_AMOUNT_ALIGN (1UL << 20)
		...
		variance_limit = ave_requested *
			reserve_mem_conf.balanced_variance_limit / 100;
