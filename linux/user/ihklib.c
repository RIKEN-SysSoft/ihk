...
static void cpu_array2str(char *str, ssize_t len,
		int num_cpus, int *cpus)
{
	...
	for (i = 0; i < num_cpus; i++) {
		if (prev_cpu != cpus[i] - 1) {
			if (prev_cpu > 0) {
				snprintf(cpu_buf, sizeof(cpu_buf) - 1,
					"%d,", prev_cpu);
				strncat(str, cpu_buf, len - strlen(str) - 1);
			}
			in_seq = 0;
		}
		else {
...
{
	cpu_array2str(str, len, req->num_cpus, req->cpus);
}

static void mem_array2str(char *str, ssize_t len,
		int num_mem_chunks, size_t *sizes, int *numa_ids)
{
	int i;
	char mem_buf[128];
	...
	for (i = 0; i < num_mem_chunks; i++) {
		snprintf(mem_buf, sizeof(mem_buf) - strlen(mem_buf) - 1,
			"%lu@%d", sizes[i], numa_ids[i]);
		strncat(str, mem_buf, len - strlen(str) - 1);
...
{
	mem_array2str(str, len, req->num_chunks,
			req->sizes, req->numa_ids);
}

void ikc_req2str(char *str, ssize_t len, ihk_ikc_req_t *req)
{
	int i, src, dst, max_dst = -1, idx;
	char cpu_str[5];
	...
	for (dst = 0; dst < IHK_MAX_NUM_CPUS; dst++) {
		...
		for (i = 0; i < ikc_sset_sizes[dst]; i++) {
			...
			strncat(str, cpu_str, len - strlen(str) - 1);
...
int ihk_reserve_cpu(int index, int* cpus, int num_cpus)
{
	...
	struct ihk_ioctl_cpu_desc req = {NULL, 0};
	int fd = -1;
...
int ihk_reserve_mem(int index, struct ihk_mem_chunk* mem_chunks, int num_mem_chunks)
{
	...
	struct ihk_ioctl_mem_desc req = {NULL, NULL, 0};
	int fd = -1;
...
{
	int ret = 0, i, ret_ioctl;
	struct ihk_ioctl_ikc_desc req = {NULL, NULL, 0};
	int fd = -1;
