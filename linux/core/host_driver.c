...
static int __ihk_os_shutdown(struct ihk_host_linux_os_data *data, int flag)
{
	...
	case IHK_OS_STATUS_NOT_BOOTED:
		pr_info("%s: info: do nothing because os is not booted\n",
			__func__);
		ret = 0;
		goto out;
	case IHK_OS_STATUS_LOADING:
	case IHK_OS_STATUS_LOADED:
...
static int __ihk_device_reserve_mem_max_ratio(
		...
		unsigned long arg)
{
	if (!data->ops || !data->ops->reserve_mem_max_ratio)
		return -1;

	return data->ops->reserve_mem_max_ratio(data, arg);
}
#endif
