...
static int __ihk_smp_release_mem_partially(size_t ihk_mem, int numa_id)
{
	...
	while (1) {
		...
		/* Releasing from the top because keeping alignment is
		 * taken care of in ihk_reserve_mem().
		 */
					 ...
					 unsigned long arg)
{
	...
	for (i = 0; i < req.num_chunks; i++) {
		if (req_sizes[i] > 0) {
			ret_internal =
				__ihk_smp_release_mem_partially(req_sizes[i],
								req_numa_ids[i]);
			dprintk(KERN_INFO "%s: ret_internal: %d\n",
				__func__, ret_internal);
			ARCHDRV_CHKANDJUMP(ret_internal != 0,
					   "__ihk_smp_release_mem_partially failed",
					   -EINVAL);
		}
	}
