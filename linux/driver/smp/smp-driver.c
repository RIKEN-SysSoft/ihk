...
static int smp_ihk_os_load_file(ihk_os_t ihk_os, void *priv, const char *fn)
{
	...
	fs = get_fs();
	set_fs(get_ds());
	printk("IHK-SMP: loading ELF header for OS 0x%lx, phys=%lx\n",
	...
	phys = (os->mem_start + IHK_SMP_LARGE_PAGE * 2 - 1) &
	        IHK_SMP_LARGE_PAGE_MASK;
	maxoffset = phys;
	...
	for(i = 0; i < elf64->e_phnum; i++){
		...
		for(size = (size + PAGE_SIZE - 1) & PAGE_MASK; size < psize;
		    size += PAGE_SIZE){
			if (offset + PAGE_SIZE > os->mem_end) {
				printk("builtin: OS is too big to load.\n");
				return -E2BIG;
...
#ifdef POSTK_DEBUG_ARCH_DEP_46 /* user area direct access fix. */
	len = strlen_user((char __user *)arg);
	karg = kzalloc(len + 1, GFP_KERNEL);
	if (!karg) {
		return -ENOMEM;
	}
...
static int smp_ihk_os_release_cpu(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
	...
	for_each_cpu_mask(cpu, cpus_to_release) {
		...
		printk(KERN_INFO "IHK-SMP: CPU HWID %d released from %p\n",
...
#ifdef POSTK_DEBUG_ARCH_DEP_46 /* user area direct access fix. */
	len = strlen_user((char __user *)arg);
	mem_string = kzalloc(len + 1, GFP_KERNEL);
	if (!mem_string) {
		return -ENOMEM;
	}

	if (copy_from_user(mem_string, (char __user *)arg, len)) {
		kfree(mem_string);
		return -EFAULT;
	}
#endif /* POSTK_DEBUG_ARCH_DEP_46 */
...
static int smp_ihk_os_release_mem(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
	...
	ret = smp_ihk_parse_mem(karg, &mem_size, &numa_id);
	kfree(karg);
#else /* POSTK_DEBUG_ARCH_DEP_46 */
...
static int smp_ihk_reserve_cpu(ihk_device_t ihk_dev, unsigned long arg)
{
	...
	cpulist_parse(karg, &cpus_to_offline);
	kfree(karg);
#else /* POSTK_DEBUG_ARCH_DEP_46 */
	/* Parse CPU list provided by user
	 * FIXME: validate userspace buffer */
	...
	for_each_cpu_mask(cpu, cpus_to_offline) {
		...
		printk(KERN_INFO "IHK-SMP: CPU %d to be offlined, HWID: %d\n",
	...
	for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
		...
		printk(KERN_INFO "IHK-SMP: CPU %d offlined successfully, HWID: %d\n",
		       ihk_smp_cpus[cpu].id, ihk_smp_cpus[cpu].hw_id);
	}
	...
	for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
		...
		printk(KERN_INFO "IHK-SMP: CPU %d reserved successfully, HWID: %d\n",
		       ihk_smp_cpus[cpu].id, ihk_smp_cpus[cpu].hw_id);
	}
...
static int smp_ihk_release_cpu(ihk_device_t ihk_dev, unsigned long arg)
{
	...
	cpulist_parse(karg, &cpus_to_online);
	kfree(karg);
#else /* POSTK_DEBUG_ARCH_DEP_46 */
	/* Parse CPU list provided by user
	 * FIXME: validate userspace buffer */
	...
	for_each_cpu_mask(cpu, cpus_to_online) {
		...
		printk("IHK-SMP: CPU %d to be onlined, HWID: %d\n",
		       ihk_smp_cpus[cpu].id, ihk_smp_cpus[cpu].hw_id);
	}
