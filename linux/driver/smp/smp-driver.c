...
static int validate_mem_req(struct ihk_mem_req *req)
{
	...
	if (req->max_size_ratio_all < 0 ||
	    IHK_RESERVE_MEM_MAX_SIZE_RATIO_ALL_LIMIT < req->max_size_ratio_all) {
		pr_err("%s: invalid max_size_ratio (%d)\n",
		       __func__, req->max_size_ratio_all);
		//ret = -EINVAL;
		//goto out;
	}
...
static int smp_ihk_os_query_mem(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
	...
	if (req.num_chunks == 0) {
		...
		pr_err("%s: error: # of chunks in output array != that of assigned\n",
			__func__);
		ret = -EINVAL;
		goto out;
	}
...
#ifdef ENABLE_PANIC_NOTIFIER_WORKAROUND
