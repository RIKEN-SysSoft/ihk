...
static void cpu_desc2str(char *str, ssize_t len, ihk_cpu_req_t *desc)
{
	cpu_array2str(str, len, desc->num_cpus, desc->cpus);
}

static int ikc_desc2str(char *str, ssize_t len, ihk_ikc_req_t *desc)
{
	char cpu_buf[8];
	int ret = 0, max_dst = 0;
	...
	for (dst = 0; dst <= max_dst; dst++) {
		...
		for (i = 0; i < desc->num_cpus; i++) {
			...
			snprintf(cpu_buf, sizeof(cpu_buf) - 1, "%d",
				src_cpus[i]);
			strcat(str, cpu_buf);
		...
		strcat(str, ":");
		snprintf(cpu_buf, sizeof(cpu_buf), "%d", dst);
		strcat(str, cpu_buf);
...
static int smp_ihk_os_query_cpu(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
	...
	int idx = 0;
	ihk_cpu_req_t req;
	ihk_cpu_req_t *res = (ihk_cpu_req_t *)arg;
	int *res_cpus = NULL;
	struct smp_os_data *os = priv;
...
static int smp_ihk_os_get_ikc_map(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
	int src, ret = 0, idx = 0;
	ihk_ikc_req_t req;
	ihk_ikc_req_t *res = (ihk_ikc_req_t *)arg;
...
static int smp_ihk_os_query_mem(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
	...
	ihk_mem_req_t req;
	ihk_mem_req_t *res = (ihk_mem_req_t *)arg;
	struct ihk_os_mem_chunk *os_mem_chunk;
