...
static struct rb_root *ihk_smp_vmap_area_root;
static spinlock_t *ihk_smp_vmap_area_lock;
...
int ihk_smp_arch_symbols_init(void)
{
	...
	ihk_smp_vmap_area_root = (void *)kallsyms_lookup_name("vmap_area_root");
	if (WARN_ON(!ihk_smp_vmap_area_root))
		return -EFAULT;

	ihk_smp_vmap_area_lock = (void *)kallsyms_lookup_name("vmap_area_lock");
	if (WARN_ON(!ihk_smp_vmap_area_lock))
		return -EFAULT;

	ihk_smp_insert_vmap_area =
		(void *)kallsyms_lookup_name("__insert_vmap_area");
	if (WARN_ON(!ihk_smp_insert_vmap_area))
		return -EFAULT;

	ihk_smp_free_vmap_area = (void *)kallsyms_lookup_name("__free_vmap_area");
	if (WARN_ON(!ihk_smp_free_vmap_area))
		return -EFAULT;
