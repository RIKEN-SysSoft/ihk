...
int main(int argc, char** argv) {
    ...
    OKNG(ret_ihklib != 0, "ihk_reserve_cpu (1)\n");
	...
	// get reserved cpus
	ret_ihklib = ihk_query_cpu(0, cpus, 1);
    OKNG(ret_ihklib != 0, "ihk_query_cpu (1)\n");

	// release cpu
	cpus[0] = 1;
	num_cpus = 1;
    ret_ihklib = ihk_release_cpu(0, cpus, num_cpus);
    OKNG(ret_ihklib != 0, "ihk_release_cpu (1)\n");
    ...
    OKNG(ret_ihklib != 0, "ihk_reserve_mem w/o /dev/mcd0\n");

	// get # of reserved mem chunks: exptected to fail
	num_mem_chunks = ihk_get_num_reserved_mem_chunks(0);
    OKNG(num_mem_chunks < 0, "ihk_get_num_reserved_mem_chunks w/o /dev/mcd0\n");

	// get reserved mem chunks: exptected to fail
	ret_ihklib = ihk_query_mem(0, mem_chunks, 1);
    OKNG(ret_ihklib != 0, "ihk_query_mem (1)\n");

	// release mem 128m@0: expected to fail
	num_mem_chunks = 1;
	mem_chunks[0].size = 128*1024*1024ULL;
	mem_chunks[0].numa_node_number = 0;
    ret_ihklib = ihk_release_mem(0, mem_chunks, num_mem_chunks);
    OKNG(ret_ihklib != 0, "ihk_release_mem w/o /dev/mcd0\n");

	// create
    ret_ihklib = ihk_create_os(0);
	OKNG(ret_ihklib != 0, "ihk_create_os (1)\n");
	...
	// get OS instances
	ret_ihklib = ihk_get_os_instances(0, indices, 1);
	...
	OKNG(ret_ihklib != 0, "ihk_destroy_os (1)\n");
	...
	ret_ihklib = ihk_query_cpu(0, cpus, num_cpus);
	OKNG(ret_ihklib == 0 &&
		 cpus[0] == 1 &&
	...
	num_cpus = 1;
    ret_ihklib = ihk_release_cpu(0, cpus, num_cpus);
    OKNG(ret_ihklib == 0, "ihk_release_cpu (2)\n");
	...
	ret_ihklib = ihk_query_mem(0, mem_chunks, num_mem_chunks);
    OKNG(ret_ihklib == 0 &&
		 (mem_chunks[0].size == 128*1024*1024ULL &&
		 mem_chunks[0].numa_node_number == 0 &&
		 mem_chunks[1].size == 64*1024*1024ULL &&
	...
	// get assigned cpus
	ret_ihklib = ihk_os_query_cpu(0, cpus, 2);
	OKNG(ret_ihklib != 0, "ihk_os_query_cpu (4)\n");

	// release cpu
	num_cpus = 1;
	cpus[0] = 1;
    ret_ihklib = ihk_os_release_cpu(0, cpus, num_cpus);
    OKNG(ret_ihklib != 0, "ihk_os_release_cpu (1)\n");
    ...
    OKNG(ret_ihklib != 0, "ihk_os_assign_mem (1)\n");
	...
	ret_ihklib = ihk_os_query_mem(0, mem_chunks, num_mem_chunks);
	OKNG(ret_ihklib != 0, "ihk_os_query_mem (1)\n");

	// release mem chunks
	num_mem_chunks = 1;
	mem_chunks[0].size = 64*1024*1024ULL;
	mem_chunks[0].numa_node_number = 0;
    ret_ihklib = ihk_os_release_mem(0, mem_chunks, num_mem_chunks);
    OKNG(ret_ihklib != 0, "ihk_os_release_mem\n");
	...
	if(atoi(argv[1]) == 0) {
		...
		ikc_map[0].src_cpu = 1;
		ikc_map[0].dst_cpu = 2;
		ret_ihklib = ihk_os_set_ikc_map(0, ikc_map, 2);
		OKNG(ret_ihklib != 0, "ihk_os_set_ikc_map\n");
		
		// get ikc_map
		ret_ihklib = ihk_os_get_ikc_map(0, ikc_map, 2);
	...
	OKNG(ret_ihklib != 0, "ihk_os_create_pseudofs\n");
	...
	// query_free_mem
	ret_ihklib = ihk_os_query_free_mem(0, memfree, num_numa_nodes);
	OKNG(ret_ihklib != 0, "ihk_os_query_free_mem\n");
	...
	// get page sizes
	ret_ihklib = ihk_os_get_pagesizes(0, pgsizes, num_pgsizes);
	OKNG(ret_ihklib != 0, "ihk_os_get_pagesizes\n");

	// get rusage
	ret_ihklib = ihk_os_getrusage(0, &rusage, sizeof(rusage));
	OKNG(ret_ihklib != 0, "ihk_os_getrusage\n");

	// shutdown
    ret_ihklib = ihk_os_shutdown(0);
	OKNG(ret_ihklib != 0, "ihk_os_shutdown (1)\n");

	// destroy os
    ret_ihklib = ihk_destroy_os(0, 0);
	OKNG(ret_ihklib != 0, "ihk_destroy_os (2)\n");

	// destroy pseudofs. Note that it doesn't check the existence of the OS.
	ret_ihklib = ihk_os_destroy_pseudofs(0);
	OKNG(ret_ihklib == 0, "ihk_os_destroy_pseudofs (2)\n");
	...
	// create 1
    ret_ihklib = ihk_create_os(0);
	OKNG(ret_ihklib == 1, "ihk_create_os (3)\n");

	// get # of OS instances
	num_os_instances = ihk_get_num_os_instances(0);
	OKNG(num_os_instances == 2, "ihk_get_num_os_instances (2)\n");

	// get OS instances. Note that the index of the youngest OS instance resides in [0].
	ret_ihklib = ihk_get_os_instances(0, indices, num_os_instances);
	OKNG(ret_ihklib == 0 &&
		 indices[0] == 1 &&
    ...
    OKNG(strstr(buf, "0") != NULL &&
		 strstr(buf, "1") != NULL, "ihkconfig 0 get os_instances (2) returned:\n%s\n", buf);

    // destroy one of them
    ret_ihklib = ihk_destroy_os(0, 1);
    OKNG(ret_ihklib == 0, "ihk_destroy_os (3)\n");
	...
	// get assigned cpus
	ret_ihklib = ihk_os_query_cpu(0, cpus, num_cpus);
	OKNG(ret_ihklib == 0 &&
		 cpus[0] == 3 &&
	...
	cpus[0] = 3;
    ret_ihklib = ihk_os_release_cpu(0, cpus, num_cpus);
    OKNG(ret_ihklib == 0, "ihk_os_release_cpu (2)\n");
	...
	cpus[0] = 1;
    ret_ihklib = ihk_os_release_cpu(0, cpus, num_cpus);
    OKNG(ret_ihklib == 0, "ihk_os_release_cpu (3)\n");
	...
	if(atoi(argv[1]) == 0) {
		...
		ikc_map[1].src_cpu = 1;
		ikc_map[1].dst_cpu = 2;
		ret_ihklib = ihk_os_set_ikc_map(0, ikc_map, num_cpus);
		OKNG(ret_ihklib == 0, "ihk_os_set_ikc_map\n");
		
		// get ikc_map
		ret_ihklib = ihk_os_get_ikc_map(0, ikc_map, num_cpus);
		OKNG(ret_ihklib == 0 &&
			 ikc_map[0].src_cpu == 3 &&
			 ikc_map[0].dst_cpu == 0 &&
			 ikc_map[1].src_cpu == 1 &&
	...
	mem_chunks[0].numa_node_number = 0;
    ret_ihklib = ihk_os_assign_mem(0, mem_chunks, num_mem_chunks);
    OKNG(ret_ihklib == 0, "ihk_os_assign_mem (3)\n");
	...
	// get status
	ret_ihklib = ihk_os_get_status(0);
	OKNG(ret_ihklib == IHK_STATUS_BOOTING ||
	...
	OKNG(strstr(buf, "BOOTING") != NULL ||
		 strstr(buf, "RUNNING") != NULL, "ihkconfig 0 get status (3) returned:\n%s\n", buf);
    ...
    ret_ihklib = ihk_os_shutdown(0);
	OKNG(ret_ihklib == 0, "ihk_os_shutdown (2)\n");
	...
	OKNG(strstr(buf, "SHUTDOWN") != NULL ||
		 strstr(buf, "INACTIVE") != NULL, "ihkconfig 0 get status (5) returned:\n%s\n", buf);
#endif
	...
	usleep(250*1000); // Wait for nothing is in-flight
    ret_ihklib = ihk_destroy_os(0, 0);
	OKNG(ret_ihklib == 0, "ihk_destroy_os (4)\n");
#else
	// destroy os
	usleep(250*1000); // Wait for nothing is in-flight
