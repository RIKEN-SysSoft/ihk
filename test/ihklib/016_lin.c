...
int main(int argc, char** argv) {
	...
	for(j = 0; j < 3; j++) {
		...
		CHKANDJUMP(1, 255, "mcexec still exist after waiting for %d us\n", i);
	no_mcexec:
		// destroy os
		for(i = 0; i < 10; i++) {
...
#define IHK_TMP "/home/takagi/project/os/install/tmp/mcos0"
		...
		nread = fread(buf, 1, sizeof(buf), fp);
		buf[nread] = 0;
		OKNG(atoi(buf) > 16384, "size of kmsg (%d) is greater than %d?\n", atoi(buf), 16384);
#endif
#if 1
		// size of local5
		...
		nread = fread(buf, 1, sizeof(buf), fp);
		buf[nread] = 0;
		OKNG(atoi(buf) >= 0, "size of local5 (%d) is >= %d?\n", atoi(buf), 0);
#endif
	}
	...
	status = system(cmd);
	CHKANDJUMP(WEXITSTATUS(status) != 0, -1, "rmmod ihk-smp-<QUOTE(ARCH)>.ko failed\n");
