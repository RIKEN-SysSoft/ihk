...
const char param[] = "IKC source CPUs";
const char *values[] = {
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < 4; i++) {
		...
		last_idx = srcs_invalid[i].ncpus - 1;

		if (i == 0) {
			srcs_invalid[i].cpus[last_idx] =
				max_id;
		}
		if (i == 1) {
			srcs_invalid[i].cpus[last_idx] =
		...
		if (i == 2) {
			srcs_invalid[i].cpus[last_idx] = -1;
		}
		if (i == 3) {
			srcs_invalid[i].cpus[last_idx] =
				INT_MAX;
