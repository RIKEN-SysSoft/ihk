...
const char param[] = "cpuacct_usage";
const char *values[] = {
	"user-mode 4s"
};
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < 1; i++) {
		...
		sprintf(cmd, "consume_cpu_time %s %s -u %d",
				fn_in, fn_out, user_time[i]);
		...
		INFO("user mode: %ld ns\n",
			ru_input_before[i].cpuacct_usage);
		...
		INFO("user mode: %ld ns\n",
				ru_input_after[i].cpuacct_usage);
