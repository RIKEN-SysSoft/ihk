...
int main(int argc, char **argv)
{
	...
	int cpu_input[] = { INT_MIN, -1, cpus.ncpus, INT_MAX };
	int errno_expected[] = { EINVAL, EINVAL, EINVAL, EINVAL };
	...
	for (i = 0; i < 4; i++) {
		...
		sprintf(cmd, "%s/bin/mcexec "
			"%s/bin/read_write_cpu_register -c 0 -a %d -F %d -e %d",
			QUOTE(WITH_MCK),
			QUOTE(CMAKE_INSTALL_PREFIX),
			ADDR_EXT, cpu_input[i], errno_expected[i]);
		ret = system(cmd);
