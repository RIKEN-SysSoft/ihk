...
const char param[] = "length(n) of the os bitmap";
const char *values[] = {
	"0",
...
int main(int argc, char **argv)
{
	...
	int n_freeze = 8 * sizeof(unsigned long) * 8;
	int n_input[2] = {
		0,
		8 * sizeof(unsigned long) * 8
	};
	...
	for (i = 0; i < 2; i++) {
		...
		ret = ihk_os_freeze(os_set[i], n_freeze);
		INTERR(ret, "ihk_os_freeze returned %d\n", ret);

		ret = os_wait_for_status(IHK_STATUS_FROZEN);
		INTERR(ret, "wait for IHK_STATUS_FROZEN timeout\n");
		...
		INFO("trying to thaw...\n");
		ret = ihk_os_thaw(os_set[i], n_input[i]);
		...
		if (ret_expected[i] == 0) {
			...
			OKNG(ret == -ETIMEDOUT, "os is still frozen\n");
			INFO("thaw with legit length of the os bitset\n");

			ret = ihk_os_thaw(os_set[i], n_freeze);
			INTERR(ret, "ihk_os_thaw returned %d\n", ret);

			ret = os_wait_for_status(IHK_STATUS_RUNNING);
			INTERR(ret, "wait for IHK_STATUS_RUNNING timeout\n");
		}
