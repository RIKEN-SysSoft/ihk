...
const char param[] = "max_num_threads";
const char *values[] = {
	"spawn 1 thread --> join"
};
...
int main(int argc, char **argv)
{
	...
	struct ihk_os_rusage ru_input[1] = { 0 };

	struct ihk_os_rusage ru_expected[3] = {
		{
			.max_num_threads = 1,
		},
		{
	...
	for (i = 0; i < 1; i++) {
		...
		sprintf(cmd, "spawn_threads %s %s -n %d",
				fn_in, fn_out, total_num_threads - 1);
		...
		OKNG(ru_input[i].max_num_threads ==
		     ru_expected[0].max_num_threads,
		     "max_num_threads: %d  expected max_num_threads: %d\n",
		     ru_input[i].max_num_threads,
		     ru_expected[0].max_num_threads);
