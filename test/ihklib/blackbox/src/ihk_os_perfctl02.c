...
struct ihk_perf_event_attr attr_input[5][1];
...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < 5; i++) {
		...
		ret = ihk_os_perfctl(os_index_input[i], PERF_EVENT_ENABLE);
		...
		ret = ihk_os_perfctl(os_index_input[i], PERF_EVENT_DISABLE);
		OKNG(ret == ret_expected[i],
		...
		ret = ihk_os_perfctl(os_index_input[i], PERF_EVENT_DESTROY);
		OKNG(ret == ret_expected[i],
