...
int main(int argc, char** argv) {
	...
	if(ikc_map_by_func) {
		...
		// set ikc_map
		sprintf(cmd, "%s/sbin/ihkosctl 0 set ikc_map 3:0+1:2 2>&1", QUOTE(MCK_DIR));
		fp = popen(cmd, "r");
		nread = fread(buf, 1, sizeof(buf), fp);
		buf[nread] = 0;
		...
		// get ikc_map
		sprintf(cmd, "%s/sbin/ihkosctl 0 get ikc_map 2>&1", QUOTE(MCK_DIR));
		fp = popen(cmd, "r");
		nread = fread(buf, 1, sizeof(buf), fp);
		buf[nread] = 0;
 ...
 shutdown:
	...
	OKNG(ret == IHK_STATUS_SHUTDOWN ||
	     ret == IHK_STATUS_INACTIVE, "ihk_os_get_status (5) returned %d\n", ret);
