...
int main(int argc, char **argv)
{
	...
	OKNG(ret == 4096, "ihk_os_get_kmsg_size\n");
#endif
	// Clear kmsg
	ret = ihk_os_clear_kmsg(0);
	OKNG(ret == 0, "ihk_os_clear_kmsg returned %d\n", ret);

	// mcexec
	sprintf(cmd, "%s/bin/mcexec ./ihklib007_mck", QUOTE(MCK_DIR));
	fp = popen(cmd, "r");
	nread = fread(buf, 1, sizeof(buf), fp);
	buf[nread] = 0;
	OKNG(strstr(buf, "McKernel process exits OK") != NULL,
	     "kprintf of many lines\n");
