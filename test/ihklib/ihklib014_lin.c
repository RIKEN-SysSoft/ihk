...
int main(int argc, char** argv) {
...
#if 0
	// run ihkmond
	status = system(PREFIX "/sbin/ihkmond -f LOG_LOCAL5 -k 0 -i -1");
	...
	// mcexec
	fp = popen(PREFIX "/bin/mcexec ./ihklib014_mck", "r");
	nread = fread(buf, 1, sizeof(buf), fp);
	buf[nread] = 0;
	...
	CHKANDJUMP(ret_ihklib < 0, 255, "ihk_os_kmsg failed");
	buf[ret_ihklib] = 0;
	...
	for(i = 0; i < 10; i++) {
		...
		if (ret_ihklib == 0) {
			OKNG(i == 0, "ihk_destroy_os (4), trial #%d succeeded\n", i + 1);
			goto destroyed;
		}
