...
struct vm_range_numa_policy *vm_range_policy_search_orig(
    struct process_vm *vm, uintptr_t addr)
{
...
struct vm_range_numa_policy *vm_range_policy_search(
    struct process_vm *vm, uintptr_t addr)
{
  if (g_ihk_test_mode != TEST_VM_RANGE_POLICY_SEARCH)  // Disable test code
    return vm_range_policy_search_orig(vm, addr);
  ...
  branch_info_t b_infos[] = {
    { 0, "invalid vm" },
    { 0, "not found any nodes" },
    { 0, "main case" },
  };

  struct vm_range_numa_policy *ret = NULL;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    root  = &vm->vm_range_numa_policy_tree;
    node = root->rb_node;
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(ret, "numa policy is found\n");
    } else {
      OKNG(!ret, "not found a valid policy\n");
    }
  }
  return ret;
 err:
