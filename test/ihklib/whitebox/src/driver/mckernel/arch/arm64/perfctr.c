...
int arm64_init_perfctr(void)
{
  ...
  if (0 < ihk_param_nr_pmu_irq_affi) {
    for (i = 0; i < ihk_param_nr_pmu_irq_affi; i++) {
      ret = ihk_mc_register_interrupt_handler(
              ihk_param_pmu_irq_affi[i], cpu_pmu.handler);
      if (ret) {
        break;
      }
    }
...
static int __ihk_mc_perfctr_init_orig(int counter, uint32_t type,
                                      uint64_t config, int mode)
{
  int ret = -1;
...
static int __ihk_mc_perfctr_init(int counter, uint32_t type,
                                 uint64_t config, int mode)
{
  if (g_ihk_test_mode != TEST__IHK_MC_PERFCTR_INIT)  // Disable test code
    return __ihk_mc_perfctr_init_orig(counter, type, config, mode);

  unsigned long ivec = 0;
  unsigned long total_branch = 4;

  branch_info_t b_infos[] = {
    { -EINVAL, "disable_counter fail" },
    { -EINVAL, "enable_intens fail" },
    { -EINVAL, "set_event_filter fail" },
    { 0,       "main case" },
  };
...
int ihk_mc_perfctr_alloc(struct thread *thread, struct mc_perf_event *event)
{
  ...
  return cpu_pmu.get_event_idx(
    counters, thread->pmc_alloc_map, event->hw_config);
}
         ...
         unsigned long pmc_status)
{
  ...
  ret = cpu_pmu.get_event_idx(
          get_per_cpu_pmu()->num_events, pmc_status, *config);
  return ret;
}
...
int ihk_mc_event_set_period(struct mc_perf_event *event)
{
  ...
  cpu_pmu.write_counter(event->counter_id, (uint64_t)(-left) & max_period);
...
uint64_t ihk_mc_event_update(struct mc_perf_event *event)
{
 ...
 again:
  ...
  if (ihk_atomic64_cmpxchg(&hwc->prev_count, prev_raw_count, new_raw_count)
        != prev_raw_count)
    goto again;
