...
static int __set_pt_page_orig(struct page_table *pt, void *virt,
                              unsigned long phys, enum ihk_mc_pt_attribute attr)
{
  ...
  ap_flag = (attr & PTATTR_FOR_USER) ? IHK_MC_AP_NOWAIT: IHK_MC_AP_CRITICAL;
...
static inline pte_t* _ptl_offset(translation_table_t* tt,
                                 unsigned long v, int level)
{
  pte_t* ptep = ptl_offset(tt, v, 4);
  if (level == 4) return ptep;
  ptep = ptl_offset(ptep, v, 3);
  if (level == 3) return ptep;
  ptep = ptl_offset(ptep, v, 2);
  if (level == 2) return ptep;
  ptep = ptl_offset(ptep, v, 1);
  return ptep;
}

static inline void _reset_ptep(translation_table_t* tt, unsigned long v,
                               int level, int reset, int free)
{
  if (!reset) return;

  pte_t* ptep = _ptl_offset(tt, v, level);
  if (free) {
    translation_table_t* _tt = (translation_table_t*)phys_to_virt(ptl_phys(ptep, level));
    if (_tt) {
      ihk_mc_free_pages(_tt, 1);
    }
  }
  ptl_clear(ptep, level);
}

static inline int _ptl_present(translation_table_t* tt, unsigned long v, int level)
{
  pte_t* ptep = _ptl_offset(tt, v, level);
  return ptl_present(ptep, level);
}

#define RESET_PTEP(tt, v, level) \
  _reset_ptep(tt, v, level, should_reset[level-1], should_free[level-1]);

static int __set_pt_page(struct page_table *pt, void *virt, unsigned long phys,
                         enum ihk_mc_pt_attribute attr)
{
  if (g_ihk_test_mode != TEST__SET_PT_PAGE)  // Disable test code
    return __set_pt_page_orig(pt, virt, phys, attr);

  unsigned long ivec = 0;
  unsigned long total_branch = 5;

  branch_info_t b_infos[] = {
    { 0,      "page table level 4 is already set" },
    { 0,      "page table level 3 is already set" },
    { -EBUSY, "page table level 2 is already set" },
    { -EBUSY, "page table level 1 is already set" },
    { 0,      "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    unsigned long v = (unsigned long)virt;
    translation_table_t* newtt = NULL;
    ihk_mc_ap_flag ap_flag;
    int in_kernel = (v >= USER_END);
    unsigned long init_pt_lock_flags;
    pte_t* ptep = NULL;
    ...
    ptep = ptl2_offset(ptep, v);
    if (ivec == 2 || attr & PTATTR_LARGEPAGE) {
      // D_Block
      if (ivec == 2 || ptl2_present(ptep)) {
        unsigned long _phys = ptl2_val(ptep) & LARGE_PAGE_MASK;
        if (ivec != 2 && (_phys == phys && ptl2_type_block(ptep))) {
          ret = 0;
        } else {  // ivec = 2
          ret = -EBUSY;
          if (ivec != 2)
    ...
    //D_Page
    ptep = ptl1_offset(ptep, v);
    if (ivec == 3 || ptl1_present(ptep)) {
      unsigned long _phys = ptl1_val(ptep) & PAGE_MASK;
      if (ivec != 3 && (_phys == phys && ptl1_type_page(ptep))) {
        ret = 0;
      } else {  // ivec = 3
        ret = -EBUSY;
        if (ivec != 3)
   ...
   out:
    ...
    if (ivec == 0) {
      OKNG(_ptl_present(tt, v, 3), "page table for level 3 is set\n");
      OKNG(_ptl_present(tt, v, 2), "page table for level 2 is set\n");
      OKNG(_ptl_present(tt, v, 1), "page table for level 1 is set\n");
    }
    if (ivec == 1) {
      OKNG(_ptl_present(tt, v, 4), "page table for level 4 is set\n");
      OKNG(_ptl_present(tt, v, 2), "page table for level 2 is set\n");
      OKNG(_ptl_present(tt, v, 1), "page table for level 1 is set\n");
    }
    if (ivec == 2) {
   ...
   err:
    if (should_quit || ivec != total_branch - 1) {
      for (level = 1; level <= 4; level++)
        RESET_PTEP(tt, v, level);
      if (should_quit) return -ENOMEM;
    }
  }
  return ret;
}

static int __clear_pt_page_orig(struct page_table *pt, void *virt, int largepage)
{
  unsigned long v = (unsigned long)virt;
  pte_t *ptep;
  translation_table_t *tt;
...
{
  if (g_ihk_test_mode != TEST__CLEAR_PT_PAGE)  // Disable test code
    return __clear_pt_page_orig(pt, virt, largepage);

  unsigned long ivec = 0;
  unsigned long total_branch = 4;

  branch_info_t b_infos[] = {
    { -EINVAL, "page table level 4 is not set" },
    { -EINVAL, "page table level 3 is not set" },
    { -EINVAL, "page table level 2 is not set" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (largepage) {
      // D_Block
      if (ivec == 2 || (!ptl2_present(ptep) || !ptl2_type_block(ptep))) {
        ret = -EINVAL;
    ...
    // D_Table
    if (ivec == 2 || (!ptl2_present(ptep) || !ptl2_type_table(ptep))) {
      ret = -EINVAL;
      if (ivec != 2) return ret;
      goto out;
    }
   ...
   out:
    ...
    OKNG(_ptl_present(tt, v, 4), "page table level 4 is not cleared\n");
    OKNG(_ptl_present(tt, v, 3), "page table level 3 is not cleared\n");

    if (largepage) {
      if (ivec == total_branch - 1) {
        OKNG(!_ptl_present(tt, v, 2), "page table level 2 is cleared\n");
      } else {
        OKNG(_ptl_present(tt, v, 2), "page table level 2 is not cleared\n");
      }
    } else {
      OKNG(_ptl_present(tt, v, 2), "page table level 2 is not cleared\n");
      if (ivec == total_branch - 1) {
        OKNG(!_ptl_present(tt, v, 1), "page table level 1 is cleared\n");
      } else {
        OKNG(_ptl_present(tt, v, 1), "page table level 1 is not cleared\n");
      }
    }
