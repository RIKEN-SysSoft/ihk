...
void profile_event_add_orig(enum profile_event_type type, uint64_t tsc)
{
...
{
  if (g_ihk_test_mode != TEST_PROFILE_EVENT_ADD)  // Disable test code
    return profile_event_add_orig(type, tsc);

  unsigned long ivec = 0;
  unsigned long total_branch = 2;

  branch_info_t b_infos[] = {
    { 0, "invalid event type" },
    { 0, "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec != 0 && type < PROFILE_EVENT_MAX) {
      event = &cpu_local_var(current)->profile_events[type];
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(event, "found the specified event\n");
    } else {
      OKNG(!event, "not found the specified event\n");
    }
  }
 err:
  return;
  ...
  unsigned long elapsed_ts)
{
  ...
  __kprintf(
    "%3s: %5s (%3s,%20s): %6s %7s offl: %6s %7s (%6s)\n",
  ...
  for (i = 0; i < PROFILE_SYSCALL_MAX; ++i) {
    ...
    __kprintf(
      ...
      profile_events[i + PROFILE_SYSCALL_MAX].cnt,
      profile_events[i + PROFILE_SYSCALL_MAX].tsc / 1000,
      (profile_events[i].tsc ? profile_events[i].tsc * 100 / elapsed_ts : 0),
      (profile_events[i].tsc ?
        (profile_events[i].tsc * 10000 / elapsed_ts) % 100 : 0)
      );
  }
  ...
  for (i = PROFILE_EVENT_MIN; i < PROFILE_EVENT_MAX; ++i) {
    ...
    __kprintf(
      ...
      profile_events[i].cnt,
      (profile_events[i].tsc / (profile_events[i].cnt ?
        profile_events[i].cnt : 1)),
      (profile_events[i].tsc && elapsed_ts ?
        profile_events[i].tsc * 100 / elapsed_ts : 0),
      (profile_events[i].tsc && elapsed_ts ?
        (profile_events[i].tsc * 10000 / elapsed_ts) % 100 : 0)
      );
  }
    ...
    struct process *proc)
{
  ...
  if (thread->profile_start_ts) {
    proc->profile_elapsed_ts += (rdtsc() - thread->profile_start_ts);
  }
...
int do_profile(int flag)
{
  ...
  if (flag & PROF_JOB) {
    ...
    if (flag & PROF_PRINT) {
      ...
      list_for_each_entry(_thread, &proc->threads_list, siblings_list) {
