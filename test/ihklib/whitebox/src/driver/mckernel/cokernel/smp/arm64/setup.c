...
int ihk_mc_get_numa_id_orig(void)
{
	if (ihk_cpu_info) {
...
{
  if (g_ihk_test_mode != TEST_IHK_MC_GET_NUMA_ID)  // Disable test code
    return ihk_mc_get_numa_id_orig();

  unsigned long ivec = 0;
  unsigned long total_branch = 2;

  branch_info_t b_infos[] = {
    { 0, "cannot get cpu info" },
    { 0, "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    START(b_infos[ivec].name);
  	...
  	if (ivec != 0 && ihk_cpu_info) {
  		ret = ihk_cpu_info->nodes[ihk_mc_get_processor_id()];
  	}	else {  // ivec = 0
  		ret = 0;
  	}
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(ret >= 0, "check numa id\n");
    } else {
      OKNG(ret == 0, "return node 0 as default\n");
    }
  }
  return ret;
 err:
