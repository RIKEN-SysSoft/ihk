...
#include <init.h>
#include <cas.h>
#include "include/rusage_private.h"
#include <syscall.h>
#include <profile.h>
#include <process.h>
#include <limits.h>
...
struct pagealloc_track_entry *__pagealloc_track_find_entry_orig(
    char *file, int line)
{
  ...
  list_for_each_entry(entry_iter, &pagealloc_track_hash[hash], hash) {
    if (!strcmp(entry_iter->file, file) && entry_iter->line == line) {
      entry = entry_iter;
      break;
    }
  }
...
struct pagealloc_track_entry *__pagealloc_track_find_entry(
    char *file, int line)
{
  if (g_ihk_test_mode != TEST__PAGEALLOC_TRACK_FIND_ENTRY)  // Disable test code
    return __pagealloc_track_find_entry_orig(file, line);

  unsigned long ivec = 0;
  unsigned long total_branch = 3;

  branch_info_t b_infos[] = {
    { 0, "tracking list is empty" },
    { 0, "not found the target entry" },
    { 0, "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 0 || list_empty(&pagealloc_track_hash[hash])) {
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(entry, "found a matched entry\n");
    } else {
      OKNG(!entry, "not found any matched entries\n");
    }
  }
...
void *_ihk_mc_alloc_aligned_pages_node_orig(
    int npages, int p2align, ihk_mc_ap_flag flag, int node,
    int is_user, uintptr_t virt_addr, char *file, int line)
{
  unsigned long irqflags;
  struct pagealloc_track_entry *entry;
  struct pagealloc_track_addr_entry *addr_entry;
  int hash, addr_hash;
  void *r = ___ihk_mc_alloc_aligned_pages_node(
              npages, p2align, flag, node, is_user, virt_addr);
...
void *_ihk_mc_alloc_aligned_pages_node(
    int npages, int p2align, ihk_mc_ap_flag flag, int node,
    int is_user, uintptr_t virt_addr, char *file, int line)
{
  if (g_ihk_test_mode != TEST__IHK_MC_ALLOC_ALIGNED_PAGES_NODE)  // Disable test code
    return _ihk_mc_alloc_aligned_pages_node_orig(
             npages, p2align, flag, node, is_user, virt_addr, file, line);

  unsigned long ivec = 0;
  unsigned long total_branch = 3;

  branch_info_t b_infos[] = {
    { 0, "page allocation tracker is not initialized" },
    { 0, "not found tracked entry" },
    { 0, "main case" },
  };

  int count_track_hash_prev = 0, count_addr_hash_prev = 0;
  int should_reset = 1;
  void *r = NULL;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    unsigned long irqflags;
    struct pagealloc_track_entry *entry, *entry_it;
    struct pagealloc_track_addr_entry *addr_entry, *addr_it;
    int hash, addr_hash;
    int count_track_hash_after = 0, count_addr_hash_after = 0;
    int entry_found = 0;
    int should_quit = 0;
    r = ___ihk_mc_alloc_aligned_pages_node(
          npages, p2align, flag, node, is_user, virt_addr);
    ...
    if (ivec == 0) {
      irqflags = ihk_mc_spinlock_lock(&pagealloc_track_hash_locks[hash]);
      list_for_each_entry(entry_it, &pagealloc_track_hash[hash], hash) {
        count_track_hash_prev++;
      }
      ihk_mc_spinlock_unlock(&pagealloc_track_hash_locks[hash], irqflags);

      irqflags = ihk_mc_spinlock_lock(&pagealloc_addr_hash_locks[addr_hash]);
      list_for_each_entry(addr_it, &pagealloc_addr_hash[addr_hash], hash) {
        count_addr_hash_prev++;
      }
      ihk_mc_spinlock_unlock(&pagealloc_addr_hash_locks[addr_hash], irqflags);
    }

    if (ivec == 0 || (!memdebug || !pagealloc_track_initialized)) {
    ...
    if (ivec == 1 || !entry) {
      ...
      dkprintf("%s entry %s:%d npages: %d added\n", __FUNCTION__,
               file, line, npages);
    } else {  // last ivec goes here
      ihk_atomic_inc(&entry->alloc_count);
    }
    ihk_mc_spinlock_unlock(&pagealloc_track_hash_locks[hash], irqflags);

    // skip re-creating addr entry
    if (ivec == total_branch - 1 && !should_reset) goto out;
   ...
   out:
    ...
    irqflags = ihk_mc_spinlock_lock(&pagealloc_track_hash_locks[hash]);
    list_for_each_entry(entry_it, &pagealloc_track_hash[hash], hash) {
      count_track_hash_after++;
      if (!strcmp(entry_it->file, file) && entry_it->line == line) {
        entry_found = 1;
      }
    }
    ihk_mc_spinlock_unlock(&pagealloc_track_hash_locks[hash], irqflags);
    ...
    irqflags = ihk_mc_spinlock_lock(&pagealloc_addr_hash_locks[addr_hash]);
    list_for_each_entry(addr_it, &pagealloc_addr_hash[addr_hash], hash) {
      count_addr_hash_after++;
    }
    ihk_mc_spinlock_unlock(&pagealloc_addr_hash_locks[addr_hash], irqflags);

    if (ivec == 0) {
      OKNG(count_addr_hash_prev == count_addr_hash_after,
           "# of addr hash entries should be unchanged\n");
    } else {
      OKNG(entry && entry->file, "entry should be tracked\n");
      OKNG(strlen(entry->file) == strlen(file), "check entry file\n");
      OKNG(entry_found, "tracked entry should be found\n");
      OKNG(addr_entry, "addr entry should be tracked\n");
      OKNG(count_addr_hash_prev + 1 == count_addr_hash_after,
           "# of addr hash entries should be increased by 1\n");
    }

    if (ivec == 1) {
      OKNG(count_track_hash_after == count_track_hash_prev + 1,
           "# of tracked entries should be increased by 1\n");
      OKNG(ihk_atomic_read(&entry->alloc_count) == 1, "check alloc count\n");
    ...
    if (ivec == 0 || ivec == total_branch - 1) {
      OKNG(count_track_hash_after == count_track_hash_prev,
           "# of tracked entries should be unchanged\n");
    }
    if (ivec == total_branch - 1) {
      if (should_reset) {
        OKNG(ihk_atomic_read(&entry->alloc_count) > 1, "check alloc count\n");
      } else {
        ihk_atomic_dec(&entry->alloc_count);
      }
    }
  }
 err:
...
static void *___ihk_mc_alloc_aligned_pages_node_orig(
    int npages, int p2align, ihk_mc_ap_flag flag,
...
static void *___ihk_mc_alloc_aligned_pages_node(
    int npages, int p2align, ihk_mc_ap_flag flag,
    int node, int is_user, uintptr_t virt_addr)
{
  if (g_ihk_test_mode != TEST___IHK_MC_ALLOC_ALIGNED_PAGES_NODE)  // Disable test code
    return ___ihk_mc_alloc_aligned_pages_node_orig(
             npages, p2align, flag, node, is_user, virt_addr);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec != 0 && pa_ops)
      ret = pa_ops->alloc_page(npages, p2align, flag, node, is_user, virt_addr);
    else {
      if (ivec == 0) ret = NULL;
      else ret = early_alloc_pages(npages);
    }
...
static void *mckernel_allocate_aligned_pages_node(
    int npages, int p2align, ihk_mc_ap_flag flag,
    int pref_node, int is_user, uintptr_t virt_addr)
{
  unsigned long pa = 0;
  int i = 0, node;
  ...
  if ((pref_node == -1) &&
    ...
    if (virt_addr != -1) {
      ...
      if (range_policy_iter) {
        range = lookup_process_memory_range(
          vm, (uintptr_t)virt_addr, ((uintptr_t)virt_addr) + 1);
        if (range) {
  ...
  if (pref_node > -1 && pref_node < ihk_mc_get_nr_numa_nodes()) {
    ...
    list_for_each_entry(pa_allocator,
      ...
      if (pa) {
        ...
        profile_event_add(PROFILE_mpol_alloc_missed, npages * PAGE_SIZE);
#endif
...
#ifdef IHK_RBTREE_ALLOCATOR
        {
          if (rusage_check_oom(numa_id, npages, is_user) == -ENOMEM) {
            ...
            pa = ihk_numa_alloc_pages(
                   &memory_nodes[memory_nodes[node].nodes_by_distance[i].id],
                   npages, p2align);
          }
#else
        list_for_each_entry(
            pa_allocator, &memory_nodes[numa_id].allocators, list) {
          if (rusage_check_oom(numa_id, npages, is_user) == -ENOMEM) {
            pa = 0;
          } else {
            pa = ihk_pagealloc_alloc(pa_allocator, npages, p2align);
  ...
  for (i = 0; i < ihk_mc_get_nr_numa_nodes(); ++i) {
...
#ifdef IHK_RBTREE_ALLOCATOR
    {
      if (rusage_check_oom(numa_id, npages, is_user) == -ENOMEM) {
        ...
        pa = ihk_numa_alloc_pages(
               &memory_nodes[(node + i) % ihk_mc_get_nr_numa_nodes()],
               npages, p2align);
      }
#else
    list_for_each_entry(pa_allocator,
...
void *ihk_mc_map_virtual_orig(unsigned long phys, int npages,
                              enum ihk_mc_pt_attribute attr)
{
  ...
  for (i = 0; i < npages; i++) {
    if (ihk_mc_pt_set_page(NULL, (char *)va + (i << PAGE_SHIFT),
      ...
      for (j = 0; j < i; j++) {
        ihk_mc_pt_clear_page(NULL, (char *)va + (j << PAGE_SHIFT));
      }
...
void *ihk_mc_map_virtual(unsigned long phys, int npages,
                         enum ihk_mc_pt_attribute attr)
{
  if (g_ihk_test_mode != TEST_IHK_MC_MAP_VIRTUAL)  // Disable test code
    return ihk_mc_map_virtual_orig(phys, npages, attr);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    for (i = 0; i < npages; i++) {
      if (ivec == 1 ||
          ihk_mc_pt_set_page(NULL, (char *)va + (i << PAGE_SHIFT),
                             phys + (i << PAGE_SHIFT), attr) != 0) {
        int j;
        for (j = 0; j < i; j++) {
          ihk_mc_pt_clear_page(NULL, (char *)va + (j << PAGE_SHIFT));
    ...
    main_ret = (char *)va + offset;
    ret = NULL;
   out:
    BRANCH_RET_CHK(ret, b_infos[ivec].expected);
    ret = main_ret;
    if (ivec == total_branch - 1) {
      OKNG(ret, "mapping address succeed\n");
    } else {
      OKNG(!ret, "mapping address fail\n");
    }
  }
  return ret;
 err:
...
void ihk_mc_unmap_virtual_orig(void *va, int npages)
{
  unsigned long i;
...
{
  if (g_ihk_test_mode != TEST_IHK_MC_UNMAP_VIRTUAL)  // Disable test code
    return ihk_mc_unmap_virtual_orig(va, npages);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    struct ihk_page_allocator_desc *desc = vmap_allocator;
    unsigned mi;
    int fail = 0;

    if (ivec == 0 || (!va || npages < 1)) {
      if (ivec != 0) return;
      goto out;
    }
   ...
   out:
    ...
    mi = (((unsigned long)va) - desc->start) >> desc->shift;
    for (i = 0; i < npages; i++, mi++) {
      if (desc->map[((mi) >> 6)] & (1UL << ((mi) & 0x3f))) {
        fail = 1; break;
      }
    }
    ...
    if (ivec == total_branch - 1) {
      OKNG(!fail, "pages are freed\n");
    } else {
      OKNG(fail, "pages are not freed\n");
    }
  }
 err:
  return;
...
void kmalloc_init(void)
{
  ...
  if (!kmalloc_track_initialized) {
    ...
    if (!memdebug) memdebug = "dummy";  // force to go inside some debug code
...
static void ___kmalloc_consolidate_list_orig(struct list_head *list)
{
...
reiterate:
  ...
  list_for_each_entry(next_chunk, list, list) {
    if (chunk_iter &&
        (((void *)chunk_iter + sizeof(struct kmalloc_header)
...
{
  if (g_ihk_test_mode != TEST__KMALLOC_CONSOLIDATE_LIST)  // Disable test code
    return ___kmalloc_consolidate_list_orig(list);
  ...
  struct kmalloc_header *next_chunk;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    struct kmalloc_header *chunk_iter, *chunk;
    int count_merge = 0, count_list_after = 0;
    int should_quit = 0;
 ...
 reiterate:
   ...
   out:
    ...
    if (ivec == total_branch - 1) {
      OKNG(count_list_prev == count_list_after + count_merge,
           "check # of chunks\n");
    } else {
      OKNG(count_list_prev == count_list_after,
           "# of chunks should be unchanged\n");
    }
...
split_and_return:
  ...
  if (chunk) {
    ...
    if (chunk->size > (size + sizeof(struct kmalloc_header))) {
      ...
      ___kmalloc_init_chunk(
        leftover, (chunk->size - size - sizeof(struct kmalloc_header)));
  ...
  ___kmalloc_init_chunk(
    chunk, (npages * PAGE_SIZE - sizeof(struct kmalloc_header)));
  ___kmalloc_insert_chunk(&cpu_local_var(free_list), chunk);
