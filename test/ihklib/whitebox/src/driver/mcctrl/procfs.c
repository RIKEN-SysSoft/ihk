...
static struct procfs_list_entry *find_procfs_entry_orig(
    struct procfs_list_entry *parent, const char *name)
{
  struct list_head *list;
...
static struct procfs_list_entry *find_procfs_entry(
    struct procfs_list_entry *parent, const char *name)
{
  if (g_ihk_test_mode != TEST_FIND_PROCFS_ENTRY)  // Disable test code
    return find_procfs_entry_orig(parent, name);

  unsigned long ivec = 0;
  unsigned long total_branch = 3;

  branch_info_t b_infos[] = {
    { 0, "not has any sub-entries" },
    { 0, "not found a valid entry" },
    { 0, "main case" },
  };

  struct procfs_list_entry *ret = NULL, *e;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    list_for_each_entry(e, list, list) {
      if (ivec == 1) continue;  // fake not found
      if (!strcmp(e->name, name)) {
        ret = e;
        break;
      }
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(ret != NULL, "found a target entry\n");
    } else {
...
static void delete_procfs_entries_orig(struct procfs_list_entry *top)
{
  struct procfs_list_entry *e;
...
static char *_find_entry_path(struct procfs_list_entry *e)
{
  if (!e) return NULL;

  char dup[1000] = "\0";
  char *path = kmalloc(1000 * sizeof(char), GFP_KERNEL);
  if (!path) return NULL;

  sprintf(path, "%s", e->name);
  struct procfs_list_entry *parent = e->parent;
  while (parent) {
    if (parent->entry) {
      sprintf(dup, "%s", path);
  ...
  sprintf(dup, "%s", path);
  sprintf(path, "/proc/%s", dup);
  return path;
}

static void delete_procfs_entries(struct procfs_list_entry *top)
{
  if (g_ihk_test_mode != TEST_DELETE_PROCFS_ENTRIES)  // Disable test code
    return delete_procfs_entries_orig(top);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 1 || list_empty(&top->children)) {
      if (ivec == 1) goto out;
    }
...
void procfs_exit_orig(int osnum)
{
  struct procfs_list_entry *e;
...
{
  if (g_ihk_test_mode != TEST_PROCFS_EXIT)  // Disable test code
    return procfs_exit_orig(osnum);

  unsigned long ivec = 0;
  unsigned long total_branch = 2;

  branch_info_t b_infos[] = {
    { 0, "not found base entry" },
    { 0, "main case" },
  };
