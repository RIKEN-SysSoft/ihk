...
static void free_node_topology_orig(struct mcctrl_usrdata *udp)
{
...
{
  if (g_ihk_test_mode != TEST_FREE_NODE_TOPOLOGY)  // Disable test code
    return free_node_topology_orig(udp);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 1 || list_empty(&udp->node_topology_list)) {
      goto out;
    }
    list_for_each_entry_safe(node, next, &udp->node_topology_list, chain) {
      list_del(&node->chain);
...
static void free_cpu_topology_one_orig(struct mcctrl_usrdata *udp,
...
static void free_cpu_topology_one(struct mcctrl_usrdata *udp,
    struct mcctrl_cpu_topology *cpu)
{
  if (g_ihk_test_mode != TEST_FREE_CPU_TOPOLOGY_ONE)  // Disable test code
    return free_cpu_topology_one_orig(udp, cpu);
  ...
  branch_info_t b_infos[] = {
    { 0, "usrdata is null" },
    { 0, "cpu topology is null" },
    { 0, "cpu cache list is empty" },
    { 0, "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 2 || list_empty(&cpu->cache_list)) {
      if (ivec == 2) goto out;
    }
    list_for_each_entry_safe(cache, next, &cpu->cache_list, chain) {
      list_del(&cache->chain);
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(list_empty(&cpu->cache_list), "cpu cache list should be empty\n");
    } else {
      OKNG(1, "nothing to do\n");
    }
  }
...
static void free_cpu_topology_orig(struct mcctrl_usrdata *udp)
{
  struct mcctrl_cpu_topology *cpu;
...
{
  if (g_ihk_test_mode != TEST_FREE_CPU_TOPOLOGY)  // Disable test code
    return free_cpu_topology_orig(udp);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 1 || list_empty(&udp->cpu_topology_list)) {
      goto out;
    }
...
void free_topology_info_orig(ihk_os_t os)
{
  struct mcctrl_usrdata *udp = ihk_host_os_get_usrdata(os);
...
{
  if (g_ihk_test_mode != TEST_FREE_TOPOLOGY_INFO)  // Disable test code
    return free_topology_info_orig(os);
