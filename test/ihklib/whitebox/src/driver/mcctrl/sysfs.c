...
static int remove_orig(struct sysfsm_node *target)
{
  int error;
  struct sysfsm_node *np;
  ...
  for (np = target; np; np = next_np) {
    while ((np->type == SNT_DIR) && !list_empty(&np->children)) {
      np = list_first_entry(&np->children, struct sysfsm_node, chain);
    }
...
static int _count_entries(struct sysfsm_node *root)
{
  if (!root) return 0;

  int count = 0;
  struct sysfsm_node *np;
  list_for_each_entry(np, &root->children, chain) {
    if (np->type == SNT_DIR) count += _count_entries(np);
    count++;
  }
  return count;
}

static int remove(struct sysfsm_node *target)
{
  if (g_ihk_test_mode != TEST_REMOVE)  // Disable test code
    return remove_orig(target);
  ...
  branch_info_t b_infos[] = {
    ...
    { -ENOENT, "unlink node error" },
    { 0,       "main case" },
  };

  int n_entries_prev = _count_entries(target);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    ...
    n_entries_after = _count_entries(target);

    if (ivec == total_branch - 1) {
      OKNG(list_empty(&target->children), "all child nodes are released\n");
    } else {
      OKNG(n_entries_prev == n_entries_after,
           "all child nodes are unchanged\n");
    }
  }
...
void sysfsm_cleanup_orig(ihk_os_t os)
{
  int error;
  ...
  if (sdp->sysfs_buf) {
    ihk_device_unmap_virtual(dev, sdp->sysfs_buf, sdp->sysfs_bufsize);
    ihk_device_unmap_memory(dev, sdp->sysfs_buf_pa, sdp->sysfs_bufsize);
    sdp->sysfs_buf = NULL;
    sdp->sysfs_buf_pa = 0;
    sdp->sysfs_buf_rpa = 0;
  }
...
{
  if (g_ihk_test_mode != TEST_SYSFSM_CLEANUP)  // Disable test code
    return sysfsm_cleanup_orig(os);

  unsigned long ivec = 0;
  unsigned long total_branch = 6;

  branch_info_t b_infos[] = {
    { 0, "mcctrl_usrdata not found" },
    { 0, "sysfsm_data is null" },
    { 0, "sysfs_buf is null" },
    { 0, "sysfs_root is null" },
    { 0, "cannot remove sysfs_root" },
    { 0, "main case" },
  };
  ...
  if (!sdp) return;
  void *sysfs_buf_prev = sdp->sysfs_buf;
  long sysfs_buf_rpa_prev = sdp->sysfs_buf_rpa;
  long sysfs_buf_pa_prev = sdp->sysfs_buf_pa;
  struct sysfsm_node *sysfs_root_prev = sdp->sysfs_root;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    ...
    if (ivec == 2 || ivec == 3 || ivec == 4) {
      OKNG(sdp->sysfs_buf == NULL && sdp->sysfs_buf_pa == 0
           && sdp->sysfs_buf_rpa == 0, "sysfs_buf is freed\n");
      OKNG(sdp->sysfs_root == NULL, "sysfs_root is reset\n");
    }
    if (ivec == total_branch - 1) {
      int os_index = ihk_host_os_get_index(os);
   ...
   err:
    if (ivec != total_branch - 1) {
      sdp->sysfs_buf = sysfs_buf_prev;
      sdp->sysfs_buf_pa = sysfs_buf_pa_prev;
      sdp->sysfs_buf_rpa = sysfs_buf_rpa_prev;
      sdp->sysfs_root = sysfs_root_prev;
    }
