...
{
  if (g_ihk_test_mode != TEST_PAGER_CLEANUP)  // Disable test code
    return pager_cleanup_orig();

  unsigned long ivec = 0;
  unsigned long total_branch = 2;

  branch_info_t b_infos[] = {
    { 0, "pager_list is empty" },
    { 0, "main case" },
  };

  unsigned long flags;
  struct pager *pager_next, *pager;
  int pager_count_prev = 0;
  spin_lock_irqsave(&pager_lock, flags);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    int pager_count_after = 0;

    spin_lock_irqsave(&pager_lock, flags);

    if (ivec == 0 || list_empty(&pager_list)) {
      goto out;
    }
   ...
   out:
    ...
    if (ivec == total_branch - 1) {
      OKNG(pager_count_after == 0, "pager list is cleared\n");
    } else {
      OKNG(pager_count_prev == pager_count_after, "do nothing\n");
    }
  }
 err:
  return;
