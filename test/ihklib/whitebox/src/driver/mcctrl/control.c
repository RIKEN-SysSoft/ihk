...
long mcctrl_perf_num_orig(ihk_os_t os, unsigned long arg)
{
  struct mcctrl_usrdata *usrdata = ihk_host_os_get_usrdata(os);
...
{
  if (g_ihk_test_mode != TEST_MCCTRL_PERF_NUM)  // Disable test code
    return mcctrl_perf_num_orig(os, arg);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    ...
    if (ivec == 0)
      OKNG(!usrdata, "user data must be null \n");
    else
      OKNG(usrdata->perf_event_num >= 0, "# of perf events should be natural number\n");
  }
  return 0;
 err:
  return -EINVAL;
...
long mcctrl_perf_set_orig(ihk_os_t os, struct ihk_perf_event_attr *__user arg)
{
  struct mcctrl_usrdata *usrdata = ihk_host_os_get_usrdata(os);
  struct ikc_scd_packet isp;
  ...
  for (i = 0; i < usrdata->perf_event_num; i++) {
    ret = copy_from_user(&attr, &arg[i], sizeof(struct ihk_perf_event_attr));
    if (ret) {
    ...
    for (j = 0; j < info->n_cpus; j++) {
      ret = mcctrl_ikc_send_wait(os, j, &isp, msecs_to_jiffies(10000),
...
{
  if (g_ihk_test_mode != TEST_MCCTRL_PERF_SET)  // Disable test code
    return mcctrl_perf_set_orig(os, arg);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    for (i = 0; i < usrdata->perf_event_num; i++) {
      ...
      kfree(perf_desc); perf_desc = NULL;
    }
   ...
   out:
    ...
    if (ivec == total_branch - 1) {
      OKNG(num_registered > 0, "check # of perf events registered\n");
    } else {
      OKNG(num_registered == 0, "no any perf events is registered\n");
    }
  }
  return ret;
 err:
...
long mcctrl_getrusage_orig(ihk_os_t ihk_os,
                           struct mcctrl_ioctl_getrusage_desc *__user _desc)
{
...
long mcctrl_getrusage(ihk_os_t ihk_os,
                      struct mcctrl_ioctl_getrusage_desc *__user _desc)
{
  if (g_ihk_test_mode != TEST_MCCTRL_GETRUSAGE)  // Disable test code
    return mcctrl_getrusage_orig(ihk_os, _desc);
  ...
  branch_info_t b_infos[] = {
    { -EINVAL, "invalid os instance" },
    { -EFAULT, "cannot get rusage" },
    { -EINVAL, "invalid # of numa nodes" },
    { -EINVAL, "invalid # of processors" },
    { -EINVAL, "invalid size of rusage" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 2 || rusage_global->num_numa_nodes < 1) {
      ret = -EINVAL;
    ...
    if (ivec == 3 || rusage_global->num_processors < 1) {
      ret = -EINVAL;
    ...
    if (ivec == 4 || desc.size_rusage > sizeof(struct ihk_os_rusage)) {
      if (ivec != 4) {
        printk("%s: desc.size_rusage=%ld > sizeof(struct mckernel_rusage)=%ld\n",
               __func__, desc.size_rusage,
               sizeof(struct ihk_os_rusage));
...
long __mcctrl_control_orig(ihk_os_t os, unsigned int req,
...
long __mcctrl_control(ihk_os_t os, unsigned int req,
                      unsigned long arg, struct file *file)
{
  if (g_ihk_test_mode != TEST__MCCTRL_CONTROL)  // Disable test code
    return __mcctrl_control_orig(os, req, arg, file);

  unsigned long ivec = 0;
  unsigned long total_branch = 3;

  branch_info_t b_infos[] = {
    { -EINVAL, "invalid os or file" },
    { -EINVAL, "invalid request" },
    { 0,       "main case" },
  };

  int ret, tmp;
  unsigned int req_prev = req;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 0 || (!os || ihk_host_validate_os(os) || !file)) {
      ret = -EINVAL;
      if (ivec != 0) return ret;
      goto out;
    }

    if (ivec == 1) req = 0;  // fake invalid request

    ret = __mcctrl_control_orig(os, req, arg, file);
    tmp = ret;
    if (ivec == total_branch - 1 && ret) ret = 0;
