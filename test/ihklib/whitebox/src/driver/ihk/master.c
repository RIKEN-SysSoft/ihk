...
void ihk_ikc_destroy_channel_orig(struct ihk_ikc_channel_desc *c)
{
    if (!c) {
        return;
    }
...
{
  if (g_ihk_test_mode != TEST_IHK_IKC_DESTROY_CHANNEL)  // Disable test code
    return ihk_ikc_destroy_channel_orig(c);
  ...
  if (!c) return;
  enum ihk_ikc_channel_flag ikc_flag_prev = c->flag;
  struct list_head *all_list = ihk_ikc_get_channel_list(c->remote_os);
  ihk_spinlock_t *all_lock = ihk_ikc_get_channel_list_lock(c->remote_os);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    ...
    if (ivec == total_branch - 1) {
      OKNG(!(c->flag & IKC_FLAG_ENABLED), "channel is disabled\n");
      OKNG(list_empty(&c->packet_pool), "packet pool should be empty\n");
      OKNG(all_list_count_prev == all_list_count_after + 1,
           "the number of items in channel list should be decreased by 1\n");
    } else {
      OKNG(c->flag == ikc_flag_prev, "ikc flag is unchanged\n");
