  ...
  const char *format, ...)
{
  ...
  if (g_ihk_test_mode != TEST_TRUNCATE_SNPRINTF) {  // Disable test code
    va_start(ap, format);
    n = truncate_snprintf_orig(str, size, format, ap);
    va_end(ap);
    return n;
  }
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    if (ivec == 0) OKNG(n <= 1, "n: %d, expected: <= 1\n", n);
    if (ivec == 1) OKNG(n == size-1, "n: %d, expected: %d\n", n, size-1);
    if (ivec == 2) {
      OKNG(n < size, "n: %d, expected: < %d\n", n, size);
      int len = strlen(str);
      OKNG(n == len, "string len: %d, expected: %d\n", len, n);
    }
  }
...
{
  if (g_ihk_test_mode != TEST_LINUX_NUMA_2_LWK_NUMA)  // Disable test code
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    if (ivec == total_branch - 1) {
      OKNG(ret >= 0, "found a valid numa mapping\n");
    } else {
      OKNG(ret == -1, "not found the node numa_id\n");
    }
  }
  return ret;
 err:
  ...
  int num_cpus, int *cpus)
{
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 0 || (!str || len <= 0 || num_cpus <= 0 || !cpus)) {
      if (ivec != 0) return 0;
      goto out;
    }
    ...
    if (ivec == 1) {
      ncpus = 8;
      _cpus = kmalloc(ncpus * sizeof(int), GFP_KERNEL);
      if (!_cpus) return -ENOMEM;
      _cpus[0] = 2; _cpus[1] = 3; _cpus[2] = 4; _cpus[3] = 5;
      _cpus[4] = 6; _cpus[5] = 7; _cpus[6] = 8; _cpus[7] = 9;
      n = cpu_array2str_orig(buf, buf_len, ncpus, _cpus);
      sprintf(target, "%s", "2-9");
    }
    ...
    if (ivec == 2) {
      ...
      _cpus = kmalloc(ncpus * sizeof(int), GFP_KERNEL);
      if (!_cpus) return -ENOMEM;
      _cpus[0] = 2; _cpus[1] = 4; _cpus[2] = 6;
      _cpus[3] = 30; _cpus[4] = 32; _cpus[5] = 34;
      n = cpu_array2str_orig(buf, buf_len, ncpus, _cpus);
      sprintf(target, "%s", "2,4,6,30,32,34");
    }
    ...
    if (ivec == 4) {
      ...
      _cpus = kmalloc(ncpus * sizeof(int), GFP_KERNEL);
      if (!_cpus) return -ENOMEM;
      _cpus[0] = 2; _cpus[1] = 3; _cpus[2] = 4; _cpus[3] = 7;
      _cpus[4] = 9; _cpus[5] = 12; _cpus[6] = 13; _cpus[7] = 14;
      n = cpu_array2str_orig(buf, buf_len, ncpus, _cpus);
      sprintf(target, "%s", "2-4,7,9,12-14");
    }
    ...
    if (ivec == 5) {
      ...
      _cpus = kmalloc(ncpus * sizeof(int), GFP_KERNEL);
      if (!_cpus) return -ENOMEM;
      _cpus[0] = 3; _cpus[1] = 5; _cpus[2] = 8; _cpus[3] = 9;
      _cpus[4] = 10; _cpus[5] = 13; _cpus[6] = 15;
      n = cpu_array2str_orig(buf, buf_len, ncpus, _cpus);
      sprintf(target, "%s", "3,5,8-10,13,15");
    }

   out:
    OKNG(n == strlen(target),
         "checking size return. expected: %d, real: %d\n", strlen(target), n);
    OKNG(strcmp(buf, target) == 0,
         "checking result string. expected: '%s', real: '%s'\n", target, buf);
    should_quit = 0;
...
static int smp_ihk_os_load_file_orig(ihk_os_t ihk_os, void *priv, const char *fn)
{
  int ret;
  struct smp_os_data *os = priv;
  struct file *file;
  ...
  if (os->bootstrap_numa_id == -1) {
    ...
    list_for_each_entry(os_mem_chunk_iter, &ihk_mem_used_chunks, list) {
      if (min_numa_id != -1 && min_numa_id <= os_mem_chunk_iter->numa_id) {
        continue;
      }
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_OS_LOAD_FILE)  // Disable test code
    return smp_ihk_os_load_file_orig(ihk_os, priv, fn);

  unsigned long ivec = 0;
  unsigned long total_branch = 13;

  branch_info_t b_infos[] = {
    { -EINVAL, "list of mem used chunks is empty" },
    { -EINVAL, "not found a valid mem chunk" },
    { -ENOENT, "filp_open fail" },
    { -EINVAL, "cannot map address for loading ELF header" },
    { -EFAULT, "read ELF header fail" },
    { -EINVAL, "not found elf header entry type PT_LOAD" },
    { -EINVAL, "all elf header entries virtual addr is zero" },
    { -E2BIG,  "OS is too big to load" },
    { -EINVAL, "cannot map address for reading data" },
    { -EFAULT, "read data fail" },
    { -E2BIG,  "OS is too big to load" },
    { -EINVAL, "setup startup fail" },
    { 0,       "main case" },
  };

  struct smp_os_data *os = priv;
  int ret, i;
  int bootstrap_numa_id_prev = os->bootstrap_numa_id;
  unsigned long flags;
  spin_lock_irqsave(&os->lock, flags);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    os->bootstrap_mem_start = 0;
    os->bootstrap_mem_end = 0;
    os->bootstrap_numa_id = bootstrap_numa_id_prev;
    ret = 0;

    if (ivec == 0) os->bootstrap_numa_id = -1;
    ...
    if (os->bootstrap_numa_id == -1) {
      ...
      if (ivec == 0 || list_empty(&ihk_mem_used_chunks)) {
        // through
      } else list_for_each_entry(os_mem_chunk_iter, &ihk_mem_used_chunks, list) {
        if (min_numa_id != -1 && min_numa_id <= os_mem_chunk_iter->numa_id) {
          continue;
        }
    ...
    for (i = 0; i < elf64->e_phnum; i++) {
      ...
      if (ivec == 5 || elf64p[i].p_type != PT_LOAD)
        continue;
      if (ivec == 6 || elf64p[i].p_vaddr == 0)
        continue;
      ...
      while (pos < end) {
        ...
        if (ivec == 7 || offset + PAGE_SIZE > os->bootstrap_mem_end) {
          if (ivec != 7) {
            printk("builtin: OS is too big to load.\n");
            should_quit = 1;
          }
      ...
      for (size = (size + PAGE_SIZE - 1) & PAGE_MASK;
           size < psize; size += PAGE_SIZE) {
        if (ivec == 10 || offset + PAGE_SIZE > os->bootstrap_mem_end) {
          if (ivec != 10) {
            printk("builtin: OS is too big to load.\n");
            should_quit = 1;
          }
    ...
    int vmap_area_taken = 0;
    if (ivec > 11 && (vmap_area_taken = smp_ihk_os_map_lwk(phys))) {
      pr_info("%s: WARNING: smp_ihk_os_map_lwk failed: %d\n",
              __func__, vmap_area_taken);
    }

    if (ivec == 11 || (ret = smp_ihk_os_setup_startup(os, phys, entry))) {
      ret = -EINVAL;
      if (ivec != 11) {
        printk("%s: ERROR: smp_ihk_os_setup_startup failed (%d)\n",
               __FUNCTION__, ret);
   ...
   out:
    ...
    if (ivec <= 1) {
      ...
      OKNG(os_status_after == BUILTIN_OS_STATUS_INITIAL,
           "os status should be initial\n");
      OKNG(os->bootstrap_mem_start > 0 &&
           os->bootstrap_mem_end > os->bootstrap_mem_start,
           "check bootstrap mem\n");
    }
    if (ivec > 4) {
      OKNG(phys > 0, "check phys\n");
    }
    if (ivec > 6) {
      OKNG(offset > 0, "check read offset\n");
    }

    if (ivec == total_branch - 1) {
      OKNG(os->boot_rip > 0, "check boot_rip\n");
      OKNG(dump_bootstrap_mem_start > 0, "check dump_bootstrap_mem_start\n");
      if (vmap_area_taken) OKNG(lwk_va, "lwk_va should be initialized\n");
    } else {
      OKNG(os->boot_rip == 0, "boot_rip should not be set\n");
      OKNG(dump_bootstrap_mem_start == 0,
           "dump_bootstrap_mem_start should not be set\n");
    }
  }
  return ret;
 err:
...
static int smp_ihk_os_load_mem_orig(ihk_os_t ihk_os, void *priv, const char *buf,
...
static int smp_ihk_os_load_mem(ihk_os_t ihk_os, void *priv, const char *buf,
                               unsigned long size, long offset)
{
  if (g_ihk_test_mode != TEST_SMP_IHK_OS_LOAD_MEM)  // Disable test code
    return smp_ihk_os_load_mem_orig(ihk_os, priv, buf, size, offset);

  unsigned long ivec = 0;
  unsigned long total_branch = 6;

  branch_info_t b_infos[] = {
    { -EINVAL, "invalid hw_id map or mem_start" },
    { -E2BIG,  "size is too big" },
    { -EBUSY,  "os status is not initial" },
    { -EINVAL, "size is zero" },
    { -ENOMEM, "cannot map virtual address" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    int ret = 0;
    unsigned long phys, to_read;
    void *virt;
    int os_status_after;

    /* We just load from the lowest address of the private memory */
    if (ivec == 0 ||
        (!CORE_ISSET_ANY(&os->cpu_hw_ids_map) || os->mem_end < os->mem_start)) {
      ret = -EINVAL;
      if (ivec != 0) {
        printk("builtin: OS is not ready to boot.\n");
        return ret;
    ...
    if (ivec == 1 || os->mem_start + size > os->mem_end) {
      ret = -E2BIG;
      if (ivec != 1) {
        printk("builtin: OS is too big to load.\n");
        return ret;
    ...
    spin_lock_irqsave(&os->lock, flags);
    if (ivec == 2 || os->status != BUILTIN_OS_STATUS_INITIAL) {
      ret = -EBUSY;
      spin_unlock_irqrestore(&os->lock, flags);
      if (ivec != 2) {
        printk("builtin: OS status is not initial.\n");
...
static void merge_mem_chunks_orig(struct list_head *chunks)
{
...
{
  if (g_ihk_test_mode != TEST_MERGE_MEM_CHUNKS)  // Disable test code
    return merge_mem_chunks_orig(chunks);

  unsigned long ivec = 0;
  unsigned long total_branch = 4;

  branch_info_t b_infos[] = {
    { 0, "invalid chunks list" },
    { 0, "chunks list is empty" },
    { 0, "not found any relevant chunks pairs" },
    { 0, "main case" },
  };
  ...
  if (!chunks) return;
  /* save previous state */
  int count_chunks_prev = 0;
  list_for_each_entry(mem_chunk, chunks, chain) {
    count_chunks_prev++;
  }
  ...
  rerun:
    list_for_each_entry_safe(mem_chunk, mem_chunk_next, chunks, chain) {
      if (ivec == 2) continue;  // not found a valid pair
      if (mem_chunk != mem_chunk_next &&
          mem_chunk_next->addr == mem_chunk->addr + mem_chunk->size &&
          mem_chunk_next->numa_id == mem_chunk->numa_id) {
        dprintf("IHK-SMP: free 0x%lx - 0x%lx and 0x%lx - 0x%lx merged\n",
   ...
   out:
    ...
    if (ivec == total_branch - 1) {
      if (count_merged) {
        OKNG(count_chunks_prev == count_chunks_after + count_merged,
             "check the number of mem chunks\n");
      }
    } else {
      OKNG(count_chunks_prev == count_chunks_after,
...
static int smp_ihk_os_get_smp_status(ihk_os_t ihk_os,
                                     int *status, int *param_status)
{
  struct ihk_host_linux_os_data *os = ihk_os;
  struct smp_os_data *_os = os->priv;
  *status = _os->status;
  if (_os->param) {
    *param_status = _os->param->status;
  } else {
    *param_status = 0;
  }
  return 0;
...
static int smp_ihk_os_set_smp_status(ihk_os_t ihk_os,
                                     int status, int param_status)
{
  struct ihk_host_linux_os_data *os = ihk_os;
  struct smp_os_data *_os = os->priv;
  _os->status = status;
  _os->param->status = param_status;
  return 0;
}
                                      ...
                                      int sleepable, int timeout)
{
  ...
  if (sleepable) {
    ...
    while ((s = smp_ihk_os_query_status(ihk_os, priv)),
            s != status && s < IHK_OS_STATUS_SHUTDOWN && timeout > 0) {
      mdelay(100);
...
static int __smp_ihk_os_assign_mem_orig(ihk_os_t ihk_os, struct smp_os_data *os,
...
static int __smp_ihk_os_assign_mem(ihk_os_t ihk_os, struct smp_os_data *os,
   size_t mem_size, int numa_id)
{
  if (g_ihk_test_mode != TEST__SMP_IHK_OS_ASSIGN_MEM)  // Disable test code
    return __smp_ihk_os_assign_mem_orig(ihk_os, os, mem_size, numa_id);
  ...
  branch_info_t b_infos[] = {
    { 0,       "mem_size is zero" },
    { -ENOMEM, "not found any chunks of node numa_id" },
    { 0,       "want = IHK_SMP_MEM_ALL" },
    { 0,       "size of max chunk is less than mem_size" },
    { 0,       "leftover page is not page compound tail" },
    { 0,       "found a mem used chunk to add the new one in front of" },
    { 0,       "main case" },
  };

  /* save previous state */
  struct chunk *mem_chunk_iter;
  int count_free_prev = 0;
  list_for_each_entry(mem_chunk_iter, &ihk_mem_free_chunks, chain) {
    count_free_prev++;
  }
  struct ihk_os_mem_chunk *os_mem_chunk_iter;
  int count_used_prev = 0;
  list_for_each_entry(os_mem_chunk_iter, &ihk_mem_used_chunks, list) {
    count_used_prev++;
  }
  size_t mem_size_prev = mem_size;
  int ret = 0;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    size_t want = mem_size;
    struct list_head to_be_assigned_chunks;
    struct list_head to_be_reverted_chunks;
    struct list_head leftover_chunks;
    int should_quit = 0;
    int count_free_after = 0;
    int count_used_after = 0;

    INIT_LIST_HEAD(&to_be_assigned_chunks);
    INIT_LIST_HEAD(&to_be_reverted_chunks);
    INIT_LIST_HEAD(&leftover_chunks);

    if (ivec == 0 || !mem_size_left) {
      if (ivec == 0) mem_size_left = 0;
    }

    if (ivec == 2) {  // fake IHK_SMP_MEM_ALL
      mem_size = mem_size_left = want = IHK_SMP_MEM_ALL;
    }
    ...
    while (mem_size_left) {
      ...
      mem_chunk_match = NULL;
      list_for_each_entry(mem_chunk_iter, &ihk_mem_free_chunks, chain) {
        if (ivec == 1 || mem_chunk_iter->numa_id != numa_id) {
          continue;
        }
      ...
      if (mem_chunk_match) {
        ...
        /* Split if there is any leftover */
        if (ivec != 3 && (ivec == 4 || mem_chunk_max->size > mem_size)) {
          struct page *pg;

          pg = virt_to_page(phys_to_virt(mem_chunk_max->addr + mem_size));
          /* Do not split compound pages though */
          if (ivec != 4 && PageTail(pg)) {
            struct page *head = compound_head(pg);
            size_t comp_size = PAGE_SIZE << compound_order(head);
    ...
    list_for_each_entry_safe(os_mem_chunk_tba_iter, os_mem_chunk_tba_next,
      ...
      os_mem_chunk_next = NULL;
      list_for_each_entry(os_mem_chunk_iter, &ihk_mem_used_chunks, list) {
        // with ivec = 5, we force to assign os_mem_chunk_next != NULL
        // then the target chunks list will be not in physical address ascending order
        if (ivec == 5 || os_mem_chunk->addr < os_mem_chunk_iter->addr) {
          os_mem_chunk_next = os_mem_chunk_iter;
          break;
        }
      }
      ...
      /* Add in front of next */
      if (os_mem_chunk_next) {  // ivec = 5 goes here
        list_add_tail(&os_mem_chunk->list, &os_mem_chunk_next->list);
        dprintf("IHK-SMP: memory 0x%lx - 0x%lx (len: %lu) @ NUMA node %d assigned to %p [in front of 0x%lx]\n",
                os_mem_chunk->size, numa_id, ihk_os, os_mem_chunk_next->addr);
      } else {
   ...
   out:
    ...
    if (ivec >= 2) {  // success
      int count_assigned = 0, count_leftover = 0;
      list_for_each_entry(os_mem_chunk_tba_iter, &to_be_reverted_chunks, list) {
        count_assigned++;
      }
      ...
      if (ivec == 4) {
        OKNG(count_leftover > 0, "leftover chunk should exist\n");
      }

      OKNG(count_free_after == count_free_prev - count_assigned + count_leftover,
           "check the number of mem free chunks\n");
      OKNG(count_used_prev + count_assigned == count_used_after,
           "check the number of mem used chunks\n");

      if (ivec != total_branch - 1) {  // revert state
        /* remove dummies from used list */
        list_for_each_entry(os_mem_chunk_tba_iter,
      ...
      if (ivec == 0 || ivec == 1) {
        OKNG(count_free_after == count_free_prev,
             "the number of mem free chunks should not be changed\n");
        OKNG(count_used_prev == count_used_after,
             "the number of mem used chunks should not be changed\n");
      }
    }
  }
...
static int smp_ihk_os_assign_mem_orig(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
  struct smp_os_data *os = priv;
  unsigned long flags;
  int ret = 0, i;
  ...
  if (copy_from_user(req_sizes, req.sizes, sizeof(size_t) * req.num_chunks)) {
  ...
  for (i = 0; i < req.num_chunks; i++) {
    ret = __smp_ihk_os_assign_mem(ihk_os, os, req_sizes[i], req_numa_ids[i]);
    if (ret != 0) {
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_OS_ASSIGN_MEM)  // Disable test code
    return smp_ihk_os_assign_mem_orig(ihk_os, priv, arg);

  unsigned long ivec = 0;
  unsigned long total_branch = 2;

  branch_info_t b_infos[] = {
    { -EINVAL, "invalid num_chunks" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    int *req_numa_ids = NULL;
    int failed_index = 0;
    int count_free_after = 0, count_used_after = 0;
    int should_quit = 0;
    ret = 0;
  ...
  out:
    ...
    if (ivec == total_branch - 1) {
      OKNG(failed_index == 0, "mem chunks are assigned w/o any errors\n");
      OKNG(count_used_after > count_used_prev,
           "the number of mem used chunks should be increased\n");
    } else {
      OKNG(count_free_after == count_free_prev,
           "the number of mem free chunks should be unchanged\n");
      OKNG(count_used_after == count_used_prev,
           "the number of mem used chunks should be unchanged\n");
    }
  }
...
static int _smp_ihk_os_release_mem_orig(ihk_os_t ihk_os, size_t size, int numa_id)
{
  int ret;
...
static int _smp_ihk_os_release_mem(ihk_os_t ihk_os, size_t size, int numa_id)
{
  if (g_ihk_test_mode != TEST__SMP_IHK_OS_RELEASE_MEM)  // Disable test code
    return _smp_ihk_os_release_mem_orig(ihk_os, size, numa_id);

  unsigned long ivec = 0;
  unsigned long total_branch = 3;

  branch_info_t b_infos[] = {
    { 0,       "list of mem used chunks is empty" },
    { -EINVAL, "not found a valid mem chunk" },
    { 0,       "main case" },
  };
  ...
  /* save previous state */
  int count_assigned_prev = 0;
  struct ihk_os_mem_chunk *os_mem_chunk;
  list_for_each_entry(os_mem_chunk, &ihk_mem_used_chunks, list) {
    count_assigned_prev++;
  }
  int count_reserved_prev = 0;
  struct chunk *mem_chunk;
  list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
    count_reserved_prev++;
  }
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    struct ihk_os_mem_chunk *next_chunk = NULL;
    struct chunk *mem_chunk;
    int count_assigned_after = 0, count_reserved_after = 0;
    int should_quit = 0;
   ...
   out:
    ...
    list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
      count_reserved_after++;
    }

    if (ivec == total_branch - 1) {
      OKNG(count_assigned_after == count_assigned_prev - 1, "the number of assigned mem chunks should be decreased by 1\n");
      OKNG(count_reserved_after == count_reserved_prev + 1, "the number of reserved mem chunks should be increased by 1\n");
    } else {
      OKNG(count_assigned_after == count_assigned_prev, "the number of assigned mem chunks should be unchanged\n");
      OKNG(count_reserved_after == count_reserved_prev, "the number of reserved mem chunks should be unchanged\n");
    }
  }
  return ret;
 err:
...
static int smp_ihk_os_release_mem_orig(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
  struct smp_os_data *os = priv;
  unsigned long flags;
  int ret = -EINVAL, i;
  ...
  ret = copy_from_user(req_sizes,
                       req.sizes, sizeof(size_t) * req.num_chunks);
  if (ret) {
  ...
  for (i = 0; i < req.num_chunks; i++) {
    ret = _smp_ihk_os_release_mem(ihk_os, req_sizes[i], req_numa_ids[i]);
    if (ret) {
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_OS_RELEASE_MEM)  // Disable test code
    return smp_ihk_os_release_mem_orig(ihk_os, priv, arg);

  unsigned long ivec = 0;
  unsigned long total_branch = 2;

  branch_info_t b_infos[] = {
    { -EINVAL, "negative num_chunks" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
   ...
   out:
    ...
    if (ivec == total_branch - 1) {
      OKNG(count_assigned_after == count_assigned_prev - req.num_chunks,
           "check the number of assigned mem chunks\n");
      OKNG(count_reserved_after == count_reserved_prev + req.num_chunks,
...
static int smp_ihk_os_get_num_mem_chunks(ihk_os_t ihk_os, void *priv)
{
  struct ihk_os_mem_chunk *os_mem_chunk;
  int count = 0;
  list_for_each_entry(os_mem_chunk, &ihk_mem_used_chunks, list) {
    if (os_mem_chunk->os != ihk_os) continue;
...
static int *smp_ihk_os_get_numa_mapping(ihk_os_t ihk_os, void *priv)
{
  struct smp_os_data *os = priv;
  return os->numa_mapping;
}

static pgd_t *smp_ihk_os_get_boot_pt(ihk_os_t ihk_os, void *priv)
{
  struct smp_os_data *os = priv;
  return os->boot_pt;
}

static int smp_ihk_os_query_mem_orig(ihk_os_t ihk_os, void *priv, unsigned long arg)
{
  ...
  if (idx > 0) {
    if (copy_to_user(req.sizes, query_res_size, sizeof(size_t) * idx)) {
      pr_err("%s: error: copying mem sizes to user-space\n", __func__);
      ret = -EFAULT;
      goto out;
    }
    if (copy_to_user(req.numa_ids, query_res_numa_id, sizeof(int) * idx)) {
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_OS_QUERY_MEM)  // Disable test code
    return smp_ihk_os_query_mem_orig(ihk_os, priv, arg);
  ...
  branch_info_t b_infos[] = {
    { -EINVAL, "negative num_chunks" },
    { 0,       "list of mem used chunks is empty" },
    { 0,       "not found any chunks of target os instance" },
    { -EINVAL, "query space is not large enough" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec != 3 && req.num_chunks == 0) {
      ...
      ret = 0;
      goto out;
    } else if (ivec == 3 || num_chunks > req.num_chunks) {
      ret = -EINVAL;
      if (ivec != 3) {
        pr_err("%s: error: query_space is not large enough\n", __func__);
        should_quit = 1;
   ...
   out:
    ...
    if (ivec == 1 || ivec == 2) {
      OKNG(idx == 0, "chunks count is zero\n");
    }
    if (ivec == total_branch - 1) {
      if (req.num_chunks > 0)
        OKNG(idx > 0, "check chunks count\n");
    }
  }
...
static struct ihk_os_ops smp_ihk_os_ops = {
  ...
  .get_smp_status = smp_ihk_os_get_smp_status,
  .set_smp_status = smp_ihk_os_set_smp_status,
  ...
  .get_num_mem_chunks = smp_ihk_os_get_num_mem_chunks,
  .get_numa_mapping = smp_ihk_os_get_numa_mapping,
  .get_boot_pt = smp_ihk_os_get_boot_pt,
...
static int smp_ihk_create_os_orig(ihk_device_t ihk_dev, void *priv,
...
static int smp_ihk_create_os(ihk_device_t ihk_dev, void *priv,
                             ...
                             struct ihk_register_os_data *regdata)
{
  if (g_ihk_test_mode != TEST_SMP_IHK_CREATE_OS)  // Disable test code
    return smp_ihk_create_os_orig(ihk_dev, priv, arg, ihk_os, regdata);
...
static int __smp_ihk_free_mem_from_rbtree_orig(struct rb_root *root)
{
  struct rb_node *node;
  struct chunk *mem_chunk;
  unsigned long size_left;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_FREE_MEM_FROM_RBTREE)  // Disable test code
    return __smp_ihk_free_mem_from_rbtree_orig(root);

  /* save previous state */
  struct rb_node *node;
  size_t count_node_before = 0;
  size_t count_released_node = 0;
  for (node = rb_first(root); node; node = rb_next(node)) {
    count_node_before++;
  }
  ...
  enum exec_path {
    PATH_NOT_COMPOUND_NOT_HEAD_PAGE    = 1UL << 0,
    PATH_FREE_COMPOUND_PAGE            = 1UL << 1,
    PATH_GO_TO_NEXT_PAGE               = 1UL << 2,
    PATH_PAGE_STRETCH_OVER_CHUNK_SIZE  = 1UL << 3,
  };
  unsigned long exec_path = 0UL;

  unsigned long ivec = 0;
  unsigned long total_ivec = 3;

  branch_info_t b_infos[] = {
    { 0, "node is null" },
    { 0, "chunk size is zero" },
    { 0, "main case" },
  };

  for (ivec = 0; ivec < total_ivec; ++ivec) {
    START(b_infos[ivec].name);
    ...
    while (node) {
      ...
      size_left = mem_chunk->size;
      size_taken = 0;
      size_t count_compound_page = 0, count_non_compound_page = 0;

      if (ivec == 1 || size_left == 0) goto out;
      ...
      while (size_left > 0) {
        ...
        if (order_size <= size_left) {
          ...
          __FUNCTION__, order_size - size_left);
          size_left = 0;
          exec_path |= PATH_PAGE_STRETCH_OVER_CHUNK_SIZE;
        }
      }
      ...
      if (exec_path & PATH_NOT_COMPOUND_NOT_HEAD_PAGE
        ...
        if (!(exec_path & PATH_FREE_COMPOUND_PAGE)) {
          OKNG(size_taken == 0,
            "size taken should be zero if the chunk doesn't contain compound pages\n");
        }
        if (exec_path & PATH_FREE_COMPOUND_PAGE) {
          if (!(exec_path & PATH_PAGE_STRETCH_OVER_CHUNK_SIZE)) {
            OKNG(size_taken < mem_chunk->size, "size taken \n");
          }
          if (exec_path & PATH_PAGE_STRETCH_OVER_CHUNK_SIZE
              && !(exec_path & PATH_NOT_COMPOUND_NOT_HEAD_PAGE)) {
            OKNG(size_taken > mem_chunk->size,
              "size taken should be greater than chunk size in case of stretched compound page\n");
          }
          if (count_compound_page > 1) {
            OKNG(exec_path & PATH_GO_TO_NEXT_PAGE, "check multiple pages\n");
          }
        }
        if (!(exec_path & PATH_PAGE_STRETCH_OVER_CHUNK_SIZE)) {
          OKNG(mem_chunk->size == size_taken + count_non_compound_page*PAGE_SIZE,
            "check size matching in case of there is no stretched page\n");
        }
      }
      if (!(exec_path & PATH_NOT_COMPOUND_NOT_HEAD_PAGE)) {
        OKNG(size_taken >= mem_chunk->size,
          "if there is no any single page and head page, size_taken should not be less than chunk size\n");
      }
   ...
   out:
    {
      /* check state */
      size_t count_node_after = 0;
      for (node = rb_first(root); node; node = rb_next(node)) {
        count_node_after++;
      }

      if (ivec == total_ivec - 1) {
        OKNG(count_node_after == 0, "the target tree should be empty\n");
        OKNG(count_released_node == count_node_before, "all nodes in the tree are processed\n");
      } else {
        OKNG(count_node_after == count_node_before, "no any nodes be left\n");
        OKNG(count_released_node == 0 && size_taken == 0, "no any chunks be freed\n");
      }
    }
  }
...
static int __ihk_smp_reserve_mem_orig(size_t ihk_mem, int numa_id,
   int min_chunk_size,
...
static void _release_mem(int numa_id)
{
	struct chunk *mem_chunk;
	struct chunk *tmp;

	list_for_each_entry_safe(mem_chunk,	tmp, &ihk_mem_free_chunks, chain) {
		if (mem_chunk->numa_id != numa_id)
			continue;
   ...
   int timeout)
{
  if (g_ihk_test_mode != TEST_IHK_SMP_RESERVE_MEM)  // Disable test code
    return __ihk_smp_reserve_mem_orig(ihk_mem, numa_id, min_chunk_size,
                                      max_size_ratio_all, timeout);

  unsigned long ivec = 0;
  unsigned long total_branch = 11;
  int ivec_movable_pages = 0;
  int ivec_free_pages = 0;
#ifdef CONFIG_MOVABLE_NODE
  ivec_movable_pages = 1;
  total_branch++;
#endif
  ...
  int ret, i;
  struct chunk *mem_chunk;
  size_t total_size_before = 0, total_size_after = 0;
  size_t chunk_count_before = 0, chunk_count_after = 0;
  list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
    if (mem_chunk->numa_id != numa_id) continue;
    total_size_before += mem_chunk->size;
    chunk_count_before++;
  }
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 0 || (order_limit < 0 || order_limit > MAX_ORDER)) {
      if (ivec != 0) {
        pr_err("IHK-SMP: error: invalid order_limit (%d)\n",
              order_limit);
        should_quit = 1;
    ...
    if (ivec == 2) __drain_all_pages = NULL;
    ...
    /* force to assign 'want' for covering all cases */
    if (ivec == total_branch-2 || ivec == total_branch-3) {
      if (want == IHK_SMP_MEM_ALL)  // need to fake size
        want = ihk_mem = 1UL << 30;
    } else {
      want = IHK_SMP_MEM_ALL;
    }
    ...
    if (want != IHK_SMP_MEM_ALL) {
      want = (ihk_mem + ((PAGE_SIZE << order) - 1)) & ~((PAGE_SIZE << order) - 1);
    }
    if (ivec >= total_branch-2)
      dprintk("%s: ihk_mem: %lu, want: %lu\n", __FUNCTION__, ihk_mem, want);
   ...
   retry:
    ...
    while (max_size_mem_chunk(&tmp_chunks) < want) {
       ...
       */
      if (ivec == 4
          || (numa_id == 0 && allocated > (available * 95 / 100))
          || (want == IHK_SMP_MEM_ALL &&
              allocated > (available * max_size_ratio_all / 100))) {
        if (ivec != 4) {
          pr_info("%s: almost all of NUMA %d taken, breaking"
      ...
      /* check 1st retry after reducing page order */
      if (ivec == 5 + ivec_movable_pages + ivec_free_pages) {
        if (order < get_order(IHK_SMP_CHUNK_BASE_SIZE)) {
          OKNG(pg, "can allocate page after reducing compound order\n");
          free_pages(page_address(pg), order);
          goto out;
        }
      }

#ifdef USE_TRY_TO_FREE_PAGES
      /* check 1st retry after calling try_to_free_pages() */
      if (ivec == 5 + ivec_movable_pages && called_free_pages) {
        OKNG(pg, "can allocate page after calling try_to_free_pages()\n");
        free_pages(page_address(pg), order);
        goto out;
      }
#endif
...
#ifdef CONFIG_MOVABLE_NODE
      /* Try movable pages if supported */
      if (ivec == 5 || (!pg && __movable_node_enabled && *__movable_node_enabled)) {
        if (pg) { free_pages(page_address(pg), order); pg = NULL; }
        if (__movable_node_enabled && *__movable_node_enabled) {
          pg = __alloc_pages_nodemask(
              __GFP_MOVABLE | __GFP_HIGHMEM |
              __GFP_COMP | __GFP_NOWARN |
              __GFP_NORETRY | __GFP_THISNODE,
        ...
        if (ivec == 5) {
          OKNG(pg != NULL, "can allocate movable page\n");
          if (pg) free_pages(page_address(pg), order);
          goto out;
        }
      }
#endif

      if ((ivec > 4 && ivec < 7 + ivec_movable_pages + ivec_free_pages)
          || !pg) {
        if (pg) free_pages(page_address(pg), order);
        ...
        if (ivec >= 5 + ivec_movable_pages + ivec_free_pages
          ...
          // simulate timeout
          if (ivec == 6 + ivec_movable_pages + ivec_free_pages) goto pre_out;
          // if not timeout then always retry
          goto retry;
        }
    ...
    pre_out:
         ...
         * multiple chunks?
         */
        if (ivec == 4 || allocated >= want || want == IHK_SMP_MEM_ALL) break;
    ...
    // test code for ivec = 2-4,6+ */
    if (ivec == 2 || ivec == 3) {  // skip draining and shrinking
      OKNG(allocated > 0, "can allocate some pages if skip draining and shrinking\n");
      goto out;
    }
    if (ivec == 4) {
      OKNG(allocated == 0, "check breaking loop condition\n");
      goto out;
    }
    if (ivec == 6 + ivec_movable_pages + ivec_free_pages) {
      OKNG(allocated == 0, "check timeout\n");
      goto out;
    }
    ...
    while (allocated < want) {
      ...
      /* Verify that chunk structure is in front of physical memory */
      if (ivec == 7 + ivec_movable_pages + ivec_free_pages
          || page_to_phys(virt_to_page(p)) != p->addr) {
        struct chunk *__p = (struct chunk *)phys_to_virt(p->addr);
        *__p = *p;
        p = __p;
        if (ivec >= total_branch-2)
      ...
      if (ivec == 7 + ivec_movable_pages + ivec_free_pages) {
        int suc = (page_to_phys(virt_to_page(p)) == p->addr);
        __ihk_smp_release_chunk(p);
        OKNG(suc, "matching chunk structure with physical memory\n");
        goto out;
      }

      /* Is the chunk bigger than what we need? */
      if (ivec == total_branch-3
          || allocated + max > want) {
        int is_leftover_compound_not_head = 0;
        int is_really_something_left = 0;
        if (ivec == total_branch-3)
          want = allocated + max - (PAGE_SIZE << 4);  // fake want
        struct page *leftover_page;
        struct chunk *leftover = (struct chunk *)(phys_to_virt(p->addr) +
          (want - allocated));
        ...
        if (PageCompound(leftover_page) && !PageHead(leftover_page)) {
          ...
          is_leftover_compound_not_head = 1;
          if (ivec == total_branch-1)
            printk("%s: adjusted leftover chunk to compound "
                   "page border: 0x%llx:%lu\n",
                   __FUNCTION__,
        ...
        if (virt_to_phys(leftover) < p->addr + max) {
          ...
          is_really_something_left = 1;
          if (ivec == total_branch-1)
            printk("%s: leftover chunk from allocation: 0x%lx:%lu\n",
                   __FUNCTION__, leftover->addr, leftover->size);
        }

        if (ivec == total_branch-3) {
          __ihk_smp_release_chunk(p);
          OKNG(is_leftover_compound_not_head, "checking execution path\n");
          if (is_really_something_left) {
            OKNG(max == p->size, "original chunk was updated\n");
          }
          goto out;
        }
      }
    ...
    /* check chunks order in the target list */
    int fail = 0;
    list_for_each_entry(q, &ihk_mem_free_chunks, chain) {
      struct chunk *_next = q->chain.next;
      if (_next == &ihk_mem_free_chunks) break;
      if (_next && q->addr >= _next->addr) {
        fail = 1; break;
      }
    }
    OKNG(!fail, "checking physical address ascending order\n");
   ...
   out:
    ...
    list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
      if (mem_chunk->numa_id != numa_id) continue;
      total_size_after += mem_chunk->size;
      chunk_count_after++;
    }

    if (ivec == total_branch-1) {  // want = IHK_SMP_MEM_ALL
      OKNG(chunk_count_after > chunk_count_before, "should have some new chunks\n");
      size_t threshold = available * max_size_ratio_all / 100;
      OKNG(total_size_after - total_size_before >= threshold,
           "checking reserved size\n");
    } else if (ivec == total_branch-2) {  // want != IHK_SMP_MEM_ALL
      OKNG(chunk_count_after > chunk_count_before, "should have some new chunks\n");
      OKNG(total_size_after - total_size_before >= want,
           "checking reserved size\n");
    } else {
...
static void __ihk_smp_release_chunk_orig(struct chunk *mem_chunk)
{
  unsigned long size_left;
  unsigned long va;
  unsigned long pa = mem_chunk->addr;
...
static void __ihk_smp_release_chunk(struct chunk *mem_chunk)
{
  if (g_ihk_test_mode != TEST_IHK_SMP_RELEASE_CHUNK)  // Disable test code
    return __ihk_smp_release_chunk_orig(mem_chunk);

  unsigned long ivec = 0;
  unsigned long total_ivec = 2;
  ...
  enum exec_path {
    PATH_NOT_COMPOUND_NOT_HEAD_PAGE    = 1UL << 0,
    PATH_FREE_COMPOUND_PAGE            = 1UL << 1,
    PATH_CHECK_SIZE_LEFT_NEXT_LOOP     = 1UL << 2,
    PATH_CHECK_SIZE_LEFT_EXIT          = 1UL << 3,
  };
  ...
  for (ivec = 0; ivec < total_ivec; ++ivec) {
    ...
    unsigned long exec_path = 0UL;
    unsigned long size_left;
    unsigned long size_taken = 0;
    size_t count_compound_page = 0;
    unsigned long va;
    unsigned long pa = mem_chunk->addr;

    va = (unsigned long)phys_to_virt(pa);
    size_left = mem_chunk->size;
    if (ivec == 0 || size_left == 0) goto out;
    ...
    while (size_left > 0) {
      ...
      if (order_size <= size_left) {
        size_left -= order_size;
        va += order_size;
        exec_path |= PATH_CHECK_SIZE_LEFT_NEXT_LOOP;
      } else {
        dprintk("%s: order_size - size_left: %lu\n",
                __func__, order_size - size_left);
        size_left = 0;
        exec_path |= PATH_CHECK_SIZE_LEFT_EXIT;
      }
    }

   out:
    if (ivec == 0) {
      OKNG(size_taken == 0, "non pages were freed\n");
      continue;
    }

    OKNG(size_taken >= mem_chunk->size, "size_taken should not be less than chunk size\n");

    if (exec_path & PATH_CHECK_SIZE_LEFT_EXIT) {
      OKNG(size_taken > mem_chunk->size,
           "size taken should be greater than chunk size in case of stretched compound page\n");
    }
    if (!(exec_path & PATH_CHECK_SIZE_LEFT_EXIT)) {
    ...
    if (count_compound_page > 0) {
      OKNG(exec_path & PATH_FREE_COMPOUND_PAGE, "should free compound page\n");
      OKNG(exec_path & PATH_CHECK_SIZE_LEFT_NEXT_LOOP
           || exec_path & PATH_CHECK_SIZE_LEFT_EXIT,
           "should check size_left\n");
    }
    if (mem_chunk->size > 0) {
      OKNG(exec_path & PATH_NOT_COMPOUND_NOT_HEAD_PAGE
           || exec_path & PATH_FREE_COMPOUND_PAGE,
           "chunk should be released\n");
    }
  }
...
static int __ihk_smp_release_mem_orig(size_t ihk_mem, int numa_id)
{
  int ret;
  struct chunk *mem_chunk;
  struct chunk *mem_chunk_next;

  list_for_each_entry_safe(mem_chunk, mem_chunk_next, &ihk_mem_free_chunks, chain) {
...
{
  if (g_ihk_test_mode != TEST_IHK_SMP_RELEASE_MEM)  // Disable test code
    return __ihk_smp_release_mem_orig(ihk_mem, numa_id);

  unsigned long ivec = 0;
  unsigned long total_branch = 3;

  branch_info_t b_infos[] = {
    { 0, "ihk_mem_free_chunks is empty" },
    { 0, "not found any chunks to release" },
    { 0, "main case" },
  };

  struct chunk *mem_chunk;
  size_t count_chunk_before = 0;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 0 || list_empty(&ihk_mem_free_chunks)) {
      if (ivec != 0) return 0;
      goto out;
    } else list_for_each_entry_safe(mem_chunk, mem_chunk_next,
                                    &ihk_mem_free_chunks, chain) {
      if (ivec == 1
          || (mem_chunk->size != ihk_mem || mem_chunk->numa_id != numa_id)) {
        continue;
      }
   ...
   out:
    {
      ...
      if (ivec == total_branch-1) {
        OKNG(count_chunk_after == count_chunk_before - 1,
             "the number of chunks should be decreased by 1\n");
        OKNG(total_size_after == total_size_before - ihk_mem,
             "total size of chunks should be decreased by %lu\n", ihk_mem);
      } else {
        OKNG(count_chunk_after == count_chunk_before,
             "the number of chunks should be unchanged\n");
        OKNG(total_size_after == total_size_before,
             "total size of chunks should not be changed\n");
      }
    }
  }
...
static int __ihk_smp_release_mem_partially_orig(size_t ihk_mem, int numa_id)
{
   int ret = -1;
   struct chunk *mem_chunk;
...
{
  if (g_ihk_test_mode != TEST_IHK_SMP_RELEASE_MEM_PARTIALLY)  // Disable test code
    return __ihk_smp_release_mem_partially_orig(ihk_mem, numa_id);
  ...
  chunk_info_t *c_it;
  struct list_head chunks_cut;
  struct chunk *chunk_shrunk = NULL;
  INIT_LIST_HEAD(&chunks_cut);
  size_t size_taken = 0;
  struct chunk *mem_chunk;

  /* save previous state */
  size_t size_left_before = ihk_mem;
  size_t total_size_before = 0;
  size_t chunk_count_before = 0;
  list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
    total_size_before += mem_chunk->size;
    chunk_count_before++;
  }
  ...
  /* for keeping track of execution path */
  enum exec_path {
    PATH_CUT_CHUNK                     = 1UL << 0,
    PATH_NOT_COMPOUND_NOT_HEAD_PAGE    = 1UL << 1,
    PATH_SKIP_SPLIT_COMPOUND_PAGE      = 1UL << 2,
    PATH_SKIP_FOR_ALIGNMENT            = 1UL << 3,
    PATH_FREE_COMPOUND_PAGE            = 1UL << 4,
    PATH_CHECK_SIZE_LEFT_NEXT_COMPOUND = 1UL << 5,
    PATH_CHECK_SIZE_LEFT_NEXT_CHUNK    = 1UL << 6,
  };
  ...
  for (ivec = 0; ivec < total_ivec; ++ivec) {
    ...
    while (1) {
      ...
      if (mem_chunk->size <= size_left) {
        ...
        /* keep track of cut chunk */
        c_it = kmalloc(sizeof(chunk_info_t), GFP_KERNEL);
        if (!c_it) return -ENOMEM;
        c_it->addr = mem_chunk->addr;
        c_it->size = mem_chunk->size;
        c_it->numa_id = mem_chunk->numa_id;
        list_add_tail(&c_it->chain, &chunks_cut);

        exec_path |= PATH_CUT_CHUNK;
        goto next_chunk;
      }
  ...
  next_compound:
        if (size_left <= 0) {
          ...
          pr_info("IHK-SMP: chunk is shrunk to 0x%lx - 0x%lx"
                  ...
                  mem_chunk->addr + mem_chunk->size,
                  mem_chunk->size, mem_chunk->numa_id);
          exec_path |= PATH_CHECK_SIZE_LEFT_NEXT_COMPOUND;
          break;
        }
      }
   ...
   out:
    ...
    /* check state */
    {
      ...
      size_t count_cut = 0, total_size_cut = 0;
      list_for_each_entry(c_it, &chunks_cut, chain) {
        total_size_cut += c_it->size;
        count_cut++;
      }
      ...
      if (ivec == total_ivec-1) {  // some memory chunks must be released or be shrunk
        ...
        /* check execution path */
        INFO("checking execution path\n");
        if (exec_path & PATH_SKIP_SPLIT_COMPOUND_PAGE
            || exec_path & PATH_SKIP_FOR_ALIGNMENT) {
 ...
 rel:
  ...
  {
    chunk_info_t *tmp;
    list_for_each_entry_safe(c_it, tmp, &chunks_cut, chain) {
      list_del(&c_it->chain);
      kfree(c_it);
    }
  }
...
static int _read_cpu_sys_file(int cpu_id, char *val)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
  ret = kernel_read(filp, val, 1, &pos);
#else
  ret = kernel_read(filp, val, 1, pos);
#endif
  if (ret <= 0) {
...
static int _smp_ihk_write_cpu_sys_file_orig(int cpu_id, char *val)
{
  struct file* filp = NULL;
  loff_t pos = 0;
  int ret, err = 0;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_WRITE_CPU_SYS_FILE)  // Disable test code
    return _smp_ihk_write_cpu_sys_file_orig(cpu_id, val);
  ...
  ret = _read_cpu_sys_file(cpu_id, status_before);
  if (ret) return -1;
...
static int smp_ihk_reserve_cpu_orig(ihk_device_t ihk_dev, unsigned long arg)
{
  int ret;
  int cpu;
  int i;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_RESERVE_CPU)
    return smp_ihk_reserve_cpu_orig(ihk_dev, arg);
  ...
  int cpu_status_before[SMP_MAX_CPUS];
  int cpu_status_after[SMP_MAX_CPUS];
  int cpu_online_before[SMP_MAX_CPUS];
  int cpu_online_after[SMP_MAX_CPUS];
  size_t count_online_before = 0;
  for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
    cpu_status_before[cpu] = ihk_smp_cpus[cpu].status;
    cpu_online_before[cpu] = cpu_online(cpu);
    if (cpu_online(cpu) && cpu_present(cpu)) count_online_before++;
  }
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    ret = copy_from_user(req_cpus, req.cpus, sizeof(int) * req.num_cpus);
    if (ret) {
      ret = -EFAULT;
      pr_err("%s: error: copying request cpus\n", __func__);
      should_quit = 1;
    ...
    for (i = 0; i < req.num_cpus; i++) {
      if (ivec == 1 || (req_cpus[i] < 0 || req_cpus[i] >= nr_cpu_ids)) {
        ret = -EINVAL;
        if (ivec != 1) {
          pr_info("%s: error: CPU %d is out of range\n", __func__, req_cpus[i]);
          should_quit = 1;
    ...
    /* Ugly, but for_each_cpu doesn't look beyond nr_cpu_ids */
    if (ivec == 2 || nr_cpu_ids >= sizeof(cpus_to_offline) * BITS_PER_BYTE) {
      ret = -EFAULT;
      if (ivec != 2) should_quit = 1;
      goto out;
    }
    for (cpu = nr_cpu_ids;
         cpu < sizeof(cpus_to_offline) * BITS_PER_BYTE; ++cpu) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,0)
      if (ivec == 3 || cpumask_test_cpu(cpu, &cpus_to_offline)) {
#else
      if (ivec == 3 || cpu_isset(cpu, cpus_to_offline)) {
#endif
        ret = -EINVAL;
        if (ivec != 3) {
          printk("%s: invalid CPU requested: %d\n",  __FUNCTION__, cpu);
    ...
    for_each_cpu_mask(cpu, cpus_to_offline) {
#endif
      if (ivec == 4 || cpu > SMP_MAX_CPUS) {
        ret = -EINVAL;
        if (ivec == 4) goto err_before_offline;
        printk("IHK-SMP: error: CPU %d is out of limit\n", cpu);
        should_quit = 1;
        goto err_before_offline;
      }

      if (ivec == 5 || !cpu_present(cpu)) {
        ret = -EINVAL;
        if (ivec == 5) goto err_before_offline;
        printk("IHK-SMP: error: CPU %d is not present\n", cpu);
        should_quit = 1;
        goto err_before_offline;
      }

      if (ivec == 6 || !cpu_online(cpu)) {
        ret = -EINVAL;
        if (ivec == 6) goto err_before_offline;
      ...
      if (ivec == 7 || ihk_smp_cpus[cpu].status != IHK_SMP_CPU_ONLINE) {
        ret = -EINVAL;
        if (ivec == 7) goto err_before_offline;
        printk("IHK-SMP: error: CPU %d is in inconsistent state, skipping\n", cpu);
        should_quit = 1;
        goto err_before_offline;
      }
    ...
    /* Offline CPU cores */
    for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
      if (ivec == 8 || ihk_smp_cpus[cpu].status != IHK_SMP_CPU_TO_OFFLINE)
        continue;

      if (ivec != 9) ret = smp_ihk_offline_cpu(cpu);
      if (ivec == 9 || ret != 0) {
        ret = -EINVAL;
        if (ivec != 9) should_quit = 1;
        goto err_during_offline;
      }
    ...
    /* Offlining CPU cores went well, mark them as available */
    for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
      if (ivec == 10 || ihk_smp_cpus[cpu].status != IHK_SMP_CPU_OFFLINED)
        continue;
      ihk_smp_cpus[cpu].status = IHK_SMP_CPU_AVAILABLE;
    ...
    if (ivec == total_branch - 1)
      printk(KERN_INFO "IHK-SMP: CPUs: %s reserved successfully\n", req_string);
    ret = 0;
    if (ivec == 8) goto err_before_offline;
    if (ivec == 10) goto err_during_offline;
    goto out;
   ...
   out:
    ...
    if (ivec == 8) {
      ...
      OKNG(!fail, "all to-be-offlined cpus must be online\n");
      for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
        if (ihk_smp_cpus[cpu].status != IHK_SMP_CPU_TO_OFFLINE)
          continue;
        ihk_smp_cpus[cpu].status = IHK_SMP_CPU_ONLINE;
    ...
    if (ivec == 10) {
      ...
      OKNG(!fail, "all to-be-offlined cpus must not be available\n");
      for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
        if (ihk_smp_cpus[cpu].status != IHK_SMP_CPU_OFFLINED)
          continue;
        smp_ihk_online_cpu(cpu);
    ...
    // get current state
    for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
      cpu_status_after[cpu] = ihk_smp_cpus[cpu].status;
      cpu_online_after[cpu] = cpu_online(cpu);
      if (cpu_online(cpu) && cpu_present(cpu)) count_online_after++;
    }
    OKNG(count_online_after > 0, "At least 1 cpu is still online (for Linux)\n");

    int fail = 0;
    if (ivec == total_branch-1) {
      // all reserved cpus must be available (with LWK) and not be online (with Linux)
      for (i = 0; i < req.num_cpus; i++) {
        cpu = req_cpus[i];
        if (ihk_smp_cpus[cpu].status != IHK_SMP_CPU_AVAILABLE
            || cpu_online(cpu)) {
      ...
      OKNG(!fail, "cpus state must be updated successfully\n");

      OKNG(req.num_cpus == count_online_before - count_online_after,
           "checking the number of online cpus\n");
    } else {
      // state must be reverted
      for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
        if (cpu_status_before[cpu] != cpu_status_after[cpu]
            || cpu_online_before[cpu] != cpu_online_after[cpu]) {
          fail = 1;
          break;
        }
      }
      OKNG(!fail, "cpus state must be unchanged\n");
    }
    should_quit = 0;
...
static int smp_ihk_release_cpu_orig(ihk_device_t ihk_dev, unsigned long arg)
{
  int ret;
  int cpu;
  int i;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_RELEASE_CPU)  // Disable test code
    return smp_ihk_release_cpu_orig(ihk_dev, arg);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    for_each_cpu_mask(cpu, cpus_to_online) {
      ...
      if (ivec == 5 || ihk_smp_cpus[cpu].status != IHK_SMP_CPU_AVAILABLE) {
        if (ivec != 5) {
          pr_err("%s: error: CPU %d isn't reserved\n", __func__, cpu);
          should_quit = 1;
        }
    ...
    /* Online CPU cores */
    for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
      if (ivec == 6 || ihk_smp_cpus[cpu].status != IHK_SMP_CPU_TO_ONLINE)
        continue;

      if (ivec != 7)
        ret = smp_ihk_online_cpu(cpu);
      if (ivec == 7 || ret != 0) {
        ret = -EINVAL;
        if (ivec != 7) should_quit = 1;
        goto revert_marking;
      }
    ...
    if (ivec == 6) { ret = -EINVAL; goto revert_marking; }
   ...
   out:
    ...
    if (ivec == total_branch-1) {
      // all released cpus must be online
      for (i = 0; i < req.num_cpus; i++) {
        cpu = req_cpus[i];
        if (ihk_smp_cpus[cpu].status != IHK_SMP_CPU_ONLINE
            || !cpu_online(cpu)) {
      ...
      OKNG(!fail, "released cpus must be online\n");

      OKNG(req.num_cpus == count_online_after - count_online_before,
        "checking the number of online cpus\n");
    } else {
...
static int smp_ihk_get_num_cpus_orig(ihk_device_t ihk_dev)
{
  int cpu;
  int num_cpus = 0;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_GET_NUM_CPUS)  // Disable test code
    return smp_ihk_get_num_cpus_orig(ihk_dev);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    num_cpus = 0;
    for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
      if (ivec == 0 || ihk_smp_cpus[cpu].status != IHK_SMP_CPU_AVAILABLE)
        continue;
   ...
   out:
    if (ivec == 0) {
      OKNG(num_cpus == 0, "count should be 0 if no any cpus available\n");
    }
    if (ivec == total_branch-1) {
      int count_offline = 0;
      for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
        if (!cpu_online(cpu) && cpu_present(cpu)) count_offline++;
      }
      OKNG(count_offline == num_cpus,
        "checking result. expected: %ld, real: %ld\n", count_offline, num_cpus);
    }
  }
...
static int smp_ihk_get_cpu_status(ihk_device_t ihk_dev, int cpu, int *status)
{
  if (cpu < 0 || cpu >= SMP_MAX_CPUS) return -EINVAL;
  if (!cpu_present(cpu)) return -EINVAL;
  switch (ihk_smp_cpus[cpu].status) {
  case IHK_SMP_CPU_AVAILABLE:
    *status = IHK_CPU_STATUS_AVAILABLE;
    return 0;

  case IHK_SMP_CPU_ASSIGNED:
    *status = IHK_CPU_STATUS_ASSIGNED;
    return 0;

  case IHK_SMP_CPU_ONLINE:
    *status = IHK_CPU_STATUS_ONLINE;
    return 0;
...
static int smp_ihk_query_cpu_orig(ihk_device_t ihk_dev, unsigned long arg)
{
  int ret;
  int idx;
  struct ihk_cpu_req req;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_QUERY_CPU)  // Disable test code
    return smp_ihk_query_cpu_orig(ihk_dev, arg);

  unsigned long ivec = 0;
  unsigned long total_branch = 5;

  branch_info_t b_infos[] = {
    { -EINVAL, "no any cpus is available" },
    { 0,       "checking the number of available cpus" },
    { -EINVAL, "invalid number of cpus requested" },
    { -EINVAL, "the number of cpus requested mismatch" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 1) goto out;  // for check counting

    if (ivec == 2 || req.num_cpus <= 0) {
      if (ivec != 2) should_quit = 1;
      ret = -EINVAL;
      goto out;
    }

    if (ivec == 3 || idx != req.num_cpus) {
      if (ivec != 3 && ivec != 0) {
        pr_err("%s: error: #cpu requested (%d) != actual (%d)\n",
               __func__, req.num_cpus, idx);
        should_quit = 1;
    ...
    res_cpus = kmalloc(sizeof(int) * req.num_cpus, GFP_KERNEL);
    if (!res_cpus) {
      pr_err("%s: error: allocating res_cpus\n", __func__);
      should_quit = 1;
      ret = -ENOMEM;
    ...
    ret = copy_to_user(req.cpus, res_cpus, sizeof(int) * req.num_cpus);
    if (ret) {
      pr_err("%s: error: copying CPU array to user-space\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
    ...
    ret = copy_to_user(&res->num_cpus, &req.num_cpus, sizeof(int));
    if (ret) {
      pr_err("%s: error: copying numer of CPUs  to user-space\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
   ...
   out:
    {
      ...
      int *cpus_offline = NULL;
      int count_offline = 0;
      for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
        if (!cpu_online(cpu) && cpu_present(cpu)) count_offline++;
      }

      if (ivec == 0) {  // no any cpus is available
        OKNG(idx == 0, "count of available cpus must equal 0\n");
      }
      /* check count of available cpus */
      if (ivec == 1) {
        OKNG(count_offline == idx,
             "the number of available cpus. expected: %ld, real: %ld\n", count_offline, idx);
      }

      /* check result */
      if (ivec == total_branch-1) {
        cpus_offline = kmalloc(count_offline * sizeof(int), GFP_KERNEL);
        if (!cpus_offline) {
          ret = -ENOMEM;
          goto err;
        }
        int i = 0;
        for (cpu = 0; cpu < SMP_MAX_CPUS; ++cpu) {
          if (cpu_online(cpu) || !cpu_present(cpu)) continue;
          cpus_offline[i] = cpu;
          i++;
        }
        int fail = 0;
        for (i = 0; i < count_offline; i++) {
          if (cpus_offline[i] != res_cpus[i]) {
            fail = 1; break;
          }
        }
        kfree(cpus_offline);
        OKNG(count_offline == req.num_cpus,
             "the number of reserved cpus. expected: %ld, real: %ld\n", i, req.num_cpus);
        OKNG(!fail, "checking result values\n");
      }

      if (res_cpus) { kfree(res_cpus); res_cpus = NULL; }
      if (should_quit) return ret;
    }
  }
...
static int smp_ihk_reserve_mem(ihk_device_t ihk_dev, unsigned long arg)
{
  ...
  if (copy_from_user(req_numa_ids, req.numa_ids, sizeof(int) * req.num_chunks)) {
    pr_err("%s: error: copying request numa_ids\n", __func__);
    ret = -EFAULT;
    goto out;
  }
  ...
  for (i = 0; i < req.num_chunks; i++) {
    ...
    if (mem_size != IHK_SMP_MEM_ALL && mem_size % IHK_RESERVE_MEM_GRANULE != 0) {
...
static int smp_ihk_release_mem_orig(ihk_device_t ihk_dev, unsigned long arg)
{
  ...
  ARCHDRV_CHKANDJUMP(req.num_chunks < 0, "invalid request length", -EINVAL);
  ...
  /* Do release */
  for (i = 0; i < req.num_chunks; i++) {
    ret = __ihk_smp_release_mem(req_sizes[i], req_numa_ids[i]);
    if (ret) {
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_RELEASE_MEM)  // Disable test code
    return smp_ihk_release_mem_orig(ihk_dev, arg);
  ...
  struct chunk *mem_chunk;
  size_t count_chunk_before = 0, count_chunk_after = 0;
  size_t total_size_before = 0, total_size_after = 0;
  list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
    count_chunk_before++;
    total_size_before += mem_chunk->size;
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    ret = copy_from_user(&req, (void *)arg, sizeof(req));
    if (ret) {
      pr_err("%s: error: copy_from_user returned %d\n", __func__, ret);
      should_quit = 1;
      ret = -EFAULT;
      goto out;
    }
    ...
    if (ivec == 0 || req.num_chunks <= 0) {
      if (ivec != 0) {
        pr_err("%s: error: invalid number of chunks\n", __func__);
        should_quit = 1;
      }
      ret = -EINVAL;
      goto out;
    ...
    req_sizes = kmalloc(sizeof(size_t) * req.num_chunks, GFP_KERNEL);
    if (!req_sizes) {
      pr_err("%s: error: cannot allocate sizes buffer\n", __func__);
      should_quit = 1;
      ret = -ENOMEM;
      goto out;
    }
    ...
    req_numa_ids = kmalloc(sizeof(int) * req.num_chunks, GFP_KERNEL);
    if (!req_numa_ids) {
      pr_err("%s: error: cannot allocate ids buffer\n", __func__);
      should_quit = 1;
      ret = -ENOMEM;
      goto out;
    }
   ...
   out:
    ...
    if (req_sizes) kfree(req_sizes);
    if (req_numa_ids) kfree(req_numa_ids);

    BRANCH_RET_CHK(ret, b_infos[ivec].expected);

    if (ivec == total_branch-1) {
      OKNG(count_chunk_after == count_chunk_before - req.num_chunks,
           "the number of chunks should be decreased by %d\n", req.num_chunks);
      OKNG(total_size_after == total_size_before - size_left,
           "total size of chunks should be decreased by %lu\n", size_left);
    } else {
      OKNG(count_chunk_after == count_chunk_before,
           "the number of chunks should be unchanged\n");
...
static int smp_ihk_release_mem_partially_orig(ihk_device_t ihk_dev,
    unsigned long arg)
{
  int ret, i;
  struct ihk_mem_req req;
...
static int smp_ihk_release_mem_partially(ihk_device_t ihk_dev,
   unsigned long arg)
{
  if (g_ihk_test_mode != TEST_SMP_IHK_RELEASE_MEM_PARTIALLY)  // Disable test code
    return smp_ihk_release_mem_partially_orig(ihk_dev, arg);
  ...
  struct list_head chunks_before;
  INIT_LIST_HEAD(&chunks_before);
  chunk_info_t *c_it;
  list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
    c_it = kmalloc(sizeof(chunk_info_t), GFP_KERNEL);
    if (!c_it) { ret = -ENOMEM; goto err; }
    c_it->addr = mem_chunk->addr;
    c_it->size = mem_chunk->size;
    c_it->numa_id = mem_chunk->numa_id;
    list_add_tail(&c_it->chain, &chunks_before);
  }
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    ret = copy_from_user(req_sizes, req.sizes, sizeof(size_t) * req.num_chunks);
    if (ret) {
      pr_err("%s: copy_from_user req_sizes\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
    ...
    ret = copy_from_user(req_numa_ids, req.numa_ids, sizeof(int) * req.num_chunks);
    if (ret) {
      pr_err("%s: copy_from_user req_numa_ids\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
    ...
    /* Do release */
    for (i = 0; i < req.num_chunks; i++) {
      if (ivec == 1 || req_sizes[i] == 0) {
        continue;
      } else {  // ivec = 2-3
        if (ivec != 2)
          ret = __ihk_smp_release_mem_partially(req_sizes[i], req_numa_ids[i]);
        if (ivec == 2 || ret) {
          if (ivec != 2) {
            pr_err("%s: __ihk_smp_release_mem_partially returned %d\n",
              __func__, ret);
            should_quit = 1;
            numa_id_has_err = req_numa_ids[i];
          }
          ret = -EINVAL;
          goto out;
        }
   ...
   out:
    ...
    if (ivec == total_branch-1) {
      if (numa_id_has_err >= 0) {  // if there is some error occurred when releasing memory
        size_t size_err_before = 0, size_err_after = 0;
        list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
          if (mem_chunk->numa_id == numa_id_has_err) size_err_after += mem_chunk->size;
        }
        list_for_each_entry(c_it, &chunks_before, chain) {
          if (c_it->numa_id == numa_id_has_err) size_err_before += c_it->size;
        }
        OKNG(size_err_after == size_err_before, "total reserved size must be unchanged if any error occurred\n");
      }
      // In success, we can not know if we must skip splitting compound page
      // or skip for better page alignment, then we can not check side effect here.
      // More checks will be implemented inside __ihk_smp_release_mem_partially()
    } else {
      OKNG(chunk_count_after == chunk_count_before, "the number of chunks must be unchanged\n");
      OKNG(total_size_after == total_size_before, "total size of chunks must be unchanged\n");
    }
 ...
 rel:
  ...
  {
    chunk_info_t *tmp;
    list_for_each_entry_safe(c_it, tmp, &chunks_before, chain) {
      list_del(&c_it->chain);
      kfree(c_it);
    }
  }
...
static int smp_ihk_query_mem_orig(ihk_device_t ihk_dev, unsigned long arg)
{
  int ret, num_chunks = 0, idx = 0;
  struct ihk_mem_req req;
  struct ihk_mem_req *res = (struct ihk_mem_req *)arg;
  ...
  if (!(query_res_size = kmalloc(sizeof(size_t) * num_chunks, GFP_KERNEL))) {
    pr_err("%s: error: allocating query_res_size\n", __func__);
    ret = -ENOMEM;
    goto out;
  }

  if (!(query_res_numa_id = kmalloc(sizeof(int) * num_chunks, GFP_KERNEL))) {
    pr_err("%s: error: allocating query_res_numa_id\n", __func__);
    ret = -ENOMEM;
    goto out;
  }
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_QUERY_MEM)  // Disable test code
    return smp_ihk_query_mem_orig(ihk_dev, arg);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    /* Count memory chunks */
    if (ivec == 1 || list_empty(&ihk_mem_free_chunks)) {
      ret = 0;
      if (ivec != 1) return copy_to_user(&res->num_chunks, &num_chunks, sizeof(int));
      goto out;
    } else list_for_each_entry(mem_chunk, &ihk_mem_free_chunks, chain) {
      num_chunks++;
    }

    if (ivec == 2 || req.num_chunks == 0) {
      /* Get reserved mem chunks */
      if (ivec != 2)
        ret = copy_to_user(&res->num_chunks, &num_chunks, sizeof(int));
      if (ret) {
        pr_err("%s: error: copying mem num_chunks to user-space\n", __func__);
        should_quit = 1;
        ret = -EFAULT;
    ...
    query_res_size = kmalloc(sizeof(size_t) * num_chunks, GFP_KERNEL);
    if (!query_res_size) {
      pr_err("%s: error: allocating query_res_size\n", __func__);
      should_quit = 1;
      ret = -ENOMEM;
    ...
    query_res_numa_id = kmalloc(sizeof(int) * num_chunks, GFP_KERNEL);
    if (!query_res_numa_id) {
      pr_err("%s: error: allocating query_res_numa_id\n", __func__);
      should_quit = 1;
      ret = -ENOMEM;
      goto out;
    ...
    ret = copy_to_user(req.sizes, query_res_size, sizeof(size_t) * idx);
    if (ret) {
      pr_err("%s: error: copying mem sizes to user-space\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
    ...
    ret = copy_to_user(req.numa_ids, query_res_numa_id, sizeof(int) * idx);
    if (ret) {
      pr_err("%s: error: copying mem numa_ids to user-space\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
    ...
    ret = copy_to_user(&res->num_chunks, &idx, sizeof(int));
    if (ret) {
      pr_err("%s: error: copying mem num_chunks to user-space\n", __func__);
      should_quit = 1;
      ret = -EFAULT;
    ...
    ret = 0;
   out:
    if (query_res_size) kfree(query_res_size);
    if (query_res_numa_id) kfree(query_res_numa_id);
    if (should_quit) return ret;
...
static struct ihk_device_ops smp_ihk_device_ops = {
  ...
  .set_test_mode = smp_ihk_set_test_mode,
  ...
  .get_cpu_status = smp_ihk_get_cpu_status,
