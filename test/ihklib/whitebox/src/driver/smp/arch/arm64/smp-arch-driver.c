...
static inline int ihk_smp_get_cpu_affinity_orig(int hwid, u64* affi)
{
...
{
  if (g_ihk_test_mode != TEST_IHK_SMP_GET_CPU_AFFINITY)  // Disable test code
    return ihk_smp_get_cpu_affinity_orig(hwid, affi);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 0 || hwid < 0 || ihk___cpu_logical_map_size <= hwid) {
      ret = -EINVAL;
      if (ivec != 0) {
        printk("IHK-SMP: invalid hwid = %d\n", hwid);
        return ret;
...
/* please refer to __test_ihk_os_query_status() for test code */
...
int smp_ihk_os_send_nmi_orig(ihk_os_t ihk_os, void *priv, int mode)
{
  struct smp_os_data *os = priv;
  int i, ret;
...
{
  if (g_ihk_test_mode != TEST_SMP_IHK_OS_SEND_NMI)  // Disable test code
    return smp_ihk_os_send_nmi_orig(ihk_os, priv, mode);
...
static int ihk_smp_cpu_kill_orig(unsigned int hwid, u64 affi)
{
  int err, i;
...
{
  if (g_ihk_test_mode != TEST_IHK_SMP_CPU_KILL)  // Disable test code
    return ihk_smp_cpu_kill_orig(hwid, affi);
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    for (i = 0; i < 10; i++) {
      if (ivec == 1) goto retry;  // fake timeout
...
int ihk_smp_reset_cpu_orig(int hw_id)
{
  int ret = 0;
  int i;
  u64 affi;
...
{
  if (g_ihk_test_mode != TEST_IHK_SMP_RESET_CPU)  // Disable test code
    return ihk_smp_reset_cpu_orig(hw_id);

  unsigned long ivec = 0;
  unsigned long total_branch = 6;

  branch_info_t b_infos[] = {
    { -EFAULT, "invalid psci ops" },
    { -EINVAL, "invalid hwid" },
    { -EINVAL, "get cpu affinity fail" },
    { -EINVAL, "not found a valid cpu" },
    { -EINVAL, "affinity info mismatch" },
    { 0,       "main case" },
  };
  ...
  for (ivec = 0; ivec < total_branch; ++ivec) {
    ...
    if (ivec == 1 || hw_id < 0 || ihk___cpu_logical_map_size <= hw_id) {
      ret = -EINVAL;
      if (ivec != 1) return ret;
      goto out;
    }
    ...
    for (i = 0; i < SMP_MAX_CPUS; i++) {
      if (ivec == 3 || (ihk_smp_cpus[i].hw_id != hw_id) ||
          (ihk_smp_cpus[i].status != IHK_SMP_CPU_ASSIGNED))
        continue;

      ret = ihk_psci_ops->affinity_info(affi, 0);
      if (ivec == 4 || ret != PSCI_0_2_AFFINITY_LEVEL_ON) {
        if (ivec == 4) {
          ret = -EINVAL;
          goto out;
        }
      } else if (ret == PSCI_0_2_AFFINITY_LEVEL_ON) {
  #if LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,0)
        ihk___smp_cross_call(cpumask_of(hw_id), INTRID_CPU_STOP);
  #else
        ihk___smp_cross_call(&cpumask_of_cpu(hw_id), INTRID_CPU_STOP);
