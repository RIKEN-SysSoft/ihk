...
int main(int argc, char **argv)
{
  ...
  char mode[6] = "\0";
  sprintf(mode, "%d", TEST_IHKLIB_OS_QUERY_MEM_SYSFS);
  ...
  unsigned long *result = malloc(mems.num_mem_chunks * sizeof(unsigned long));
  ret = ihk_os_query_total_mem(os_index, result, mems.num_mem_chunks);
  INTERR(ret, "ihklib_os_query_mem returned %d\n", ret);
