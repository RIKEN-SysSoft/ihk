 ...
 */
/* ihk_host_driver.h COPYRIGHT FUJITSU LIMITED 2019 */
...
enum ihk_cpu_status {
  IHK_CPU_STATUS_NA = 0,       /* N/A. Some hole in apic id, etc. */
  IHK_CPU_STATUS_AVAILABLE,
  IHK_CPU_STATUS_ASSIGNED,
  IHK_CPU_STATUS_ONLINE,
  IHK_CPU_STATUS_OFFLINE,
};
...
struct ihk_os_ops {
  ...
  /* dummy ops for testing wait_for_status() */
  int (*get_smp_status)(ihk_os_t, int *status, int *param_status);
  int (*set_smp_status)(ihk_os_t os, int status, int param_status);
  ...
  **/
  ...
  /* Get the number of mem chunks assigned to an os instance */
  int (*get_num_mem_chunks)(ihk_os_t, void *);

  /* Get numa-mapping */
  int *(*get_numa_mapping)(ihk_os_t, void *);

  /* Get bootstrap page tables */
  pgd_t *(*get_boot_pt)(ihk_os_t, void *);
...
struct ihk_device_ops {
   ...
   * \param arg     Test mode
   */
  int (*set_test_mode)(ihk_device_t, unsigned long arg);
  ...
  int (*get_cpu_status)(ihk_device_t ihk_dev, int cpu, int *status);
...
/** \brief Get kernel callback handlers for an OS instance */
