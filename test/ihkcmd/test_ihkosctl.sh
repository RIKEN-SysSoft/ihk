...
out=`sudo ${SBIN_DIR}/ihkosctl 0 $* 2>&1`
prev_ret=$?
...
if [ ${errcnt} -eq 0 ]; then
	echo "[`printf %03d ${case_num}`:OK] ihkosctl 0 $* , ret:${prev_ret} out:\"${out}\""
else
	echo "[`printf %03d ${case_num}`:NG] ihkosctl 0 $* , ret:${prev_ret} out:\"${out}\""
fi
